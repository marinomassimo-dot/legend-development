...
**Start here** → [FAQ: what it does, how to start it, why it is different](FAQ.md) · [Mission and objectives](disease-models/wwox/mission.md) · [What it can do: 23 skills + 5 agents](SKILLS.md) · [Explore the WWOX model](disease-models/wwox/disease_model.md) · [Architecture](ARCHITECTURE.md) · [Capabilities census](CAPABILITIES.md) · [Evaluation spec](framework/eval/README.md) · [Use it for another disease](framework/ADOPTING.md)
...
- **Grow** — a capability scout that records every real capability gap (fixed when worth it, `CHANGE = NONE` otherwise), and an error archive that turns each observed mistake into a reusable test case.
...
The 23 skills in [`.claude/skills/`](.claude/skills/) are executable
...
| [`.claude/skills/`](.claude/skills/) | 23 reusable workflows for intake, full-text analysis, discovery, evaluation, integrity and harness scouting |
...
| [`SKILLS.md`](SKILLS.md) | **The catalogue of what this system can do** — 23 skills, 5 agents, each with an honest maturity status |
