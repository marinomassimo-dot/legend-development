...
| **T7's RESULT** — *"can the procedure silently create or promote a standing root Orchestrator?" → NO, PASS* | **WITHDRAWN, and the question with it — ADJUDICATED, not decided by Plan.** The PASS rested on artefacts revision 4 deleted: at CONTENT_TIP `UNTIL RESOLVED` returns **0 hits** in `BOOTSTRAP.md`, `STOP` returns **0**, and step 9 sits at line 100 reading *"Acquire the `ORCHESTRATOR_LEASE` … and only if every condition above passed"*. Plan additionally held the question **mis-framed**: revision 4 seats a root-resident Orchestrator **openly**, by ratified intent, which is the opposite of *silently*. Plan routed the re-framing for adjudication rather than making it inside the evidence block. **Ruling — `OPEN-REV-ORCHSURF-MIRROR-002-ADD-004` § 4, branch `orchestrator` @ dd434d5: withdrawal stands; T7 is neither re-run nor re-framed inside this candidate.** Grounds, the first decisive: the answer is now *entailed by the design under test*, so the test no longer discriminates and a PASS would credit this candidate with evidence it did not earn; re-running verbatim would return PASS for a reason unrelated to the recorded one, which is a second wrong-reason pass and is what §9's own `WRONG-REASON LOAD-BEARING PASSES 0` forbids; and the residual question that *would* discriminate is the enforcement question, which the ratified record §1 item 5 defers to ROOTGUARD |
