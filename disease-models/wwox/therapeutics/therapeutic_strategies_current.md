...
- **Next action:** (1) measure the RNA output in variant-carrying cells using a published exon-8→9 assay as a template to redesign; (2) define which transcript/protein is recoverable; (3) only then run an ASO design walk or evaluate editing; (4) map n-of-1 channels with the clinical team.
 ...
 - 🔴 **MISURATO 2026-09-21 — l'endpoint di efficacia di questa strategia NON HA UN DENOMINATORE DI BASE, in nessun allele WWOX, in nessun tessuto.** Censimento vincolato della letteratura sui trascritti di splicing (audit [`wwox_splice_transcript_census_20260921.md`](../analysis/wwox_splice_transcript_census_20260921.md); lettura `FTR-20260921-39101447-01`): su 20 record rilevanti, **tre** hanno misurato un trascritto da un allele di splicing WWOX e **due** lo hanno soltanto **annotato** — sei pazienti su due alleli canonici a cui **nessuno ha mai guardato l'RNA**. Uno solo dei tre è raggiungibile da qui.
   - **Che cosa è stato misurato, nell'unico leggibile** (`PMID 39101447`, `c.172+1G>C`, **donatore** canonico — una classe diversa dall'accettore): **minigene in HEK293T, non RNA del paziente**, con introne 1 accorciato artificialmente; il prodotto è **skipping dell'esone 2**. Su tutte e quattro le misure del censimento **ogni prodotto riportato è uno skipping**: nessuno riporta ritenzione d'introne né sito criptico.
   - 🔴 **Nessun allele di splicing WWOX è mai stato saggiato con un inibitore di NMD** — `cycloheximide`, `emetine`, `actinomycin`, `nonsense-mediated` = **zero** (in tondo, informativi). Quindi **la frazione degradata contro stabile è ignota per ogni allele**, e questa strategia ne ha bisogno per sapere che cosa stia correggendo.
   - 🔴 **Non esiste alcun rapporto aberrante:normale, da nessuna parte.** Nessuna quantificazione, nessuna densitometria; il contenuto quantitativo dell'unico risultato leggibile è *"The gel-electrophoresis of RT-PCR revealed a band for wild-type and another for mutant-type."* **L'endpoint di efficacia di TX-001 — la frazione di trascritto correttamente spliced — non ha oggi alcuna misura di partenza contro cui confrontarsi.**
   - 🔴 **E non esiste alcun braccio ASO o di correzione dello splicing in tutta la letteratura WWOX:** `antisense`, `oligonucleotide` = **zero**. **TX-001 non ha alcun precedente intragenico.** Il precedente *Milasen* resta valido come precedente **regolatorio e di procedura**, non come precedente su questo gene.
   - **Nessuna proteina è mai stata osservata da un allele di splicing WWOX.** ⚠️ E nell'unico paper leggibile la proteina è stata **tradotta in software** (*"Snapgene software was used to translate nucleotide sequences into protein sequences"*) mentre la Conclusione afferma *"A minigene assay … revealed that the variation resulted in protein truncation"* — **un saggio minigene non può rivelare un troncamento proteico**. Terza istanza odierna di ipotesi ri-vocalizzata come reperto.
   - ✅ **Lead tracciabile, registrato come lead e non come piano:** il gruppo di Genova (`PMID 35573960`) **possiede fibroblasti WOREE ed esegue RT-PCR sui trascritti WWOX**; e **dieci pazienti su due alleli canonici** — la coorte fondatrice argentina `c.107+1G>A` (`PMID 42721537`, cinque pazienti, `FT-110`) e la coorte `c.606-1G>A` (`PMID 26345274`, cinque pazienti, pacchetto `A8`) — **non hanno mai avuto l'RNA esaminato**. Un solo esperimento economico — RT-PCR su fibroblasti a cavallo della giunzione, ± cicloesimide, con quantificazione della banda — **fornirebbe a questa strategia il denominatore che le manca e convertirebbe [[discovery_ledger_current#DL-BIO-002 — Trascritto/proteina dell'allele di sito accettore, nominati dalla predizione di skipping esone 9 (SpliceAI/Pangolin)|DL-BIO-002]] da in-silico a misurato**.
   - **Il gate obbligatorio qui sopra non cambia: si rafforza.** Era già scritto che nessuna scelta ASO-vs-editing precede la RT-PCR giunzione-specifica; ora si sa che **quella misura non esiste per nessun allele WWOX**, quindi il gate non è prudenza, è l'unico primo passo disponibile.
