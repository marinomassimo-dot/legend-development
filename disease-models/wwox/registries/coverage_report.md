...
|---|---:|---:|---|
| **Full text depth** | 77 | 18% | complete receipt or legacy registry declaration; trace split below |
| Partial full text | 10 | 2% | some sections read; explicitly declared incomplete |
| Abstract / screened | 13 | 3% | classified from metadata and abstract; no full text read |
| Catalogued only | 335 | 76% | known, deduplicated, never analytically processed — **the debt** |
| Filtered / superseded | 3 | 1% | explicitly set aside, with the reason preserved |

- **82** promoted `PAPER` records · **356** `CORPUS` placeholders
- **397** lifecycle entries in the literature tracking log
- **391** unique PMIDs known across the registries

## Receipt trace

- Authoritative ledger: `disease-models/wwox/registries/fulltext_read_receipts.jsonl`
- **156** append-only events: **131** contemporaneous · **22** conservative legacy reconstructions · **1** invalidation(s) · **2** identity correction(s)
- **64** registry records have a persisted `complete_fulltext_read` receipt
- **13** records still rely on a historical registry full-text declaration without a surviving complete coverage receipt
- **13** receipt event(s) do not yet map to a registry record
