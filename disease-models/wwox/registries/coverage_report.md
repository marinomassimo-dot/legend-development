...
|---|---:|---:|---|
| **Full text depth** | 71 | 17% | complete receipt or legacy registry declaration; trace split below |
...
| Catalogued only | 329 | 77% | known, deduplicated, never analytically processed — **the debt** |
...
- **58** registry records have a persisted `complete_fulltext_read` receipt
- **13** records still rely on a historical registry full-text declaration without a surviving complete coverage receipt
- **21** receipt event(s) do not yet map to a registry record
