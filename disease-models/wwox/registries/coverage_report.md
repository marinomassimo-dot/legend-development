...
|---|---:|---:|---|
| **Full text depth** | 44 | 11% | complete receipt or legacy registry declaration; trace split below |
| Partial full text | 5 | 1% | some sections read; explicitly declared incomplete |
| Abstract / screened | 16 | 4% | classified from metadata and abstract; no full text read |
| Catalogued only | 341 | 83% | known, deduplicated, never analytically processed — **the debt** |
| Filtered / superseded | 3 | 1% | explicitly set aside, with the reason preserved |

- **53** promoted `PAPER` records · **356** `CORPUS` placeholders
...
- **87** append-only events: **64** contemporaneous · **22** conservative legacy reconstructions
- **26** registry records have a persisted `complete_fulltext_read` receipt
- **18** records still rely on a historical registry full-text declaration without a surviving complete coverage receipt
- **9** receipt event(s) do not yet map to a registry record
