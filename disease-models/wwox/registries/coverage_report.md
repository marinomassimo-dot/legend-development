...
|---|---:|---:|---|
| **Full text depth** | 47 | 11% | complete receipt or legacy registry declaration; trace split below |
| Partial full text | 5 | 1% | some sections read; explicitly declared incomplete |
| Abstract / screened | 15 | 4% | classified from metadata and abstract; no full text read |
| Catalogued only | 339 | 83% | known, deduplicated, never analytically processed — **the debt** |
...
- **95** append-only events: **72** contemporaneous · **22** conservative legacy reconstructions
- **30** registry records have a persisted `complete_fulltext_read` receipt
- **17** records still rely on a historical registry full-text declaration without a surviving complete coverage receipt
- **10** receipt event(s) do not yet map to a registry record
