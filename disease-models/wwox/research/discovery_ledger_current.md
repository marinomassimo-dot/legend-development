...
### DL-METH-094 — **Non sono gli autori a perdere l'attenuazione: è la figura-modello. E la prova è interna a un solo paper**
...
- 🔴 **FALSIFICA `DL-METH-091` NELLA SUA FORMA FORTE, ED È IL MOTIVO PER CUI QUESTA LETTURA VALEVA.** Avevo concluso che *«il commentary di un autore sul proprio paper non è un riassunto: la modalità si degrada»*. Questa è una **review dello stesso laboratorio sul proprio studio primario**, e il testo corrente **attenua correttamente e ripetutamente**: *«Our recent study **suggests** new mechanisms»* · *«The first mechanism **we propose**»* · *«it is **possible to speculate**»* · *«is **still to be investigated**»*. **L'autorialità non predice l'erosione.**
- 🔴 **CHE COSA LA PREDICE: LA FIGURA-MODELLO — e il confronto è INTERNO.** Il paragrafo che introduce la Figura 1 attenua **tre volte in due frasi**. La didascalia immediatamente sotto enuncia gli stessi tre meccanismi **piatti**: *«WWOX modulates ER expression and function, which in turn positively or negatively affects p53»*, senza alcun modale. E il disegno non porta **né tratteggio, né punto interrogativo, né una parola oltre le sei etichette dei nodi**. Stessi autori, stesso anno, stessa rivista, **stesso paragrafo**: controlla tutto ciò che servirebbe a una spiegazione basata su chi scrive o su dove. **L'erosione non è fra documenti: è fra il testo corrente e il diagramma dentro un solo documento.**
- **E la seconda metà replica `PMID 27308504` esattamente.** L'unico punto in cui *questa* figura attenua è `ER→p53`, disegnato con **due frecce opposte simultanee** — un'inibizione e un'attivazione dalla stessa scatola alla stessa scatola — perché lì gli autori non hanno una rivendicazione direzionale. Su `27308504` l'unica attenuazione dell'intera figura era un `?` sul complesso MRN, **l'unico elemento che non era rivendicazione loro**. Due paper indipendenti, un laboratorio, **stessa regola: la figura segnala incertezza dove gli autori non hanno interesse, ed enuncia piatto dove ce l'hanno.**
...
**Il primo fenotipo eterozigote che questo corpus àncora**, e va nella direzione **opposta** al trasferimento segnalato su `27308504`: *«aged germline Wwox-heterozygous mice in mammary susceptible C3H genetic background develop mammary tumors with ~50% penetrance»*. Là uno schema stampava `WWOX+/−` su evidenza interamente knockdown e nulla; qui un sistema eterozigote **esiste ed è misurato**. 🔴 **Ma le condizioni al contorno pesano quanto il numero:** tumori **mammari**, in un ceppo **scelto per suscettibilità mammaria**, in animali **anziani**. Un fenotipo oncologico in un fondo sensibilizzato **non è** un'affermazione sui portatori di un genotipo neurosviluppo, e non va trasferito come tale.

**Una terza voce, non letta, su un conflitto che abbiamo già aperto.** Qui si riporta che il laboratorio Huebner mostra WWOX legare BRCA1 tramite il primo dominio WW e sostenere il **NHEJ** come via dominante di riparazione dei DSB. `DL-METH-084` porta già il conflitto HDR/NHEJ fra `25331887` e `38499540`: questo è un **terzo laboratorio indipendente** sullo stesso asse. Accodato in `FT-062`, non assorbito.
