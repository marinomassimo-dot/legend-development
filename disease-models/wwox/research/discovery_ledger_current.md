...
- **Why it is a lead and not noise.** `mRNA preserved + protein lost` is the signature of a **turnover** problem rather than a transcriptional one, and it is the exact shape this repository has already paid for once: the P252A finding (normal mRNA, accelerated turnover, **no** rescue with MG-132, rescue with lysosomal probes, HSC70 co-IP) that `gold_is_in_the_details.md` carries as its worked example. The Discussion of this very paper says *"Alteration of WWOX in cancer has also been shown to be associated with LOH, HD and enhanced protein degradation"* — citing its reference 5 — and **never applies it to its own panel**, where the candidate is sitting.
...
- **`REVIVAL_TRIGGER`:** a cycloheximide chase in SAOS-2 with proteasomal versus lysosomal/CMA inhibitors, or any loading-normalised WWOX protein quantification in SAOS-2 paired with its transcript. Either would promote this from `IPOTESI` to a route or kill it.
