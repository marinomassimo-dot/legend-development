...
- 🟢 **AGGIORNAMENTO DI STATUS — 2026-09-21. Belief: medio → MEDIO-ALTO sulla classe di meccanismo. La predizione centrale di questa voce ha ora un'istanza DIMOSTRATA, su una sostituzione missenso nel dominio SDR di WWOX.** Fonte: `PMID 41124647` (Zhang X *et al.*, senior Hong B / Wang H, *Adv Sci* 2025;13(1):e07602, PMCID PMC12767083, **Chinese Academy of Sciences, Hefei — laboratorio indipendente da Chang/NCKU**), letto integralmente; ricevuta `FTR-20260921-41124647-01`; audit [`wwox_missense_cma_degradation_audit_20260921.md`](../analysis/wwox_missense_cma_degradation_audit_20260921.md). Citazioni verificate personalmente contro l'artefatto.
  - ✅ **Ciò che questa voce aveva predetto per ragionamento strutturale è ora osservato:** per `p.P252A`, **l'mRNA non scende e la proteina sì** — la perdita è **post-trascrizionale**, esattamente la forma che questa voce aveva nominato (*"proteina instabile/degradata, non enzima cataliticamente morto-ma-presente"*). La via è **lisosomiale, NON proteasomiale, NON macroautofagica**: *"the proteasome inhibitor, MG-132 treatment, **did not induce** the accumulation of WWOX … proteins"*, 3-MA negativo, cloroquina e NH₄Cl positivi.
  - 🟢 **E la via è BLOCCABILE, il che aggiunge un braccio concreto all'esperimento già scritto sopra:** cloroquina **40 µM, 24 h** e NH₄Cl **250 µM, 24 h** ripristinano il livello proteico del mutante. ⚠️ **Ma in un solo sistema e su un transgene**: CAL-62 (carcinoma tiroideo) che **sovraesprime stabilmente** un costrutto Flag, letto come intensità di banda al Western. **E nessun saggio funzionale è stato eseguito sotto rescue** — né clonogenicità, né invasione, né comet, né γH2AX, né NHEJ. **La via è bloccabile; se bloccarla ripristini qualcosa è non testato.**
  - 🔴 **Non importare «autofagia mediata da chaperone» come stabilita.** Gli autori stessi scrivono *"Since the KFERQ-like motif (LRSVQ at 187-191 amino acids) exists on WWOX, **we speculated** that lysosomal degradation … occurred through chaperone-mediated autophagy"* — e l'abstract la riporta come dimostrata. **Il motivo KFERQ non è mai stato mutato**, e `LAMP2A`/`LAMP2` — il recettore che *definisce* la CMA — compaiono **zero volte** nel corpo (parole in tondo, zeri informativi), così come `bafilomycin`. Il legame **HSC70 è misurato** (co-IP, mutante-positivo / wild-type-negativo), ma nulla lo collega a *quel* motivo se non un docking. **Registrare: degradazione lisosomio-dipendente, proteasoma-indipendente, macroautofagia-indipendente, con co-IP HSC70 e motivo KFERQ-like PREDETTO.** È la **seconda** occorrenza in un solo giorno del re-voicing di un'ipotesi della fonte come suo reperto.
  - 🔴 **E c'è un freno umano che questa voce deve portare, perché va nella direzione opposta all'entusiasmo.** Il probando è un uomo di **35 anni, omozigote per due sostituzioni missenso del dominio SDR**, i cui mutanti sono **indistinguibili dal vettore vuoto** su clonogenicità, vitalità, invasione, migrazione, xenograft, comet, γH2AX e un reporter NHEJ — e tuttavia, verbatim: *"the cancer patient harboring germline homozygous P252A and P282A variants **did not suffer from WWOX-related nervous system disease**."* La lettura degli autori è quella che va adottata: *"However, WWOX and WWOX mutants **may have some residual protein function to maintain the development of the neurological system**."* **«Morto ai saggi oncologici» ≠ «nullo neurologicamente».** Qualunque inferenza che leghi la perdita di funzione misurata in un saggio tumorale alla gravità neurologica attesa **va fermata qui**.
  - ⚠️ **Distanza dichiarata, e non va coperta:** la collocazione dei due residui nello span SDR è **aritmetica del lettore, non del paper** — `SDR`, `ADH` e `short-chain` compaiono **zero volte** nel corpo. Il paper non nomina mai il dominio. Inoltre **solo `P252A` è l'allele degradato**; `P282A` perde funzione **senza difetto di stabilità** ed è il caso più pulito. Per `P252A` **abbondanza e funzione sono confuse e mai separate**: il paper stesso spiega la perdita del co-IP con POLE4 *"likely due to the **low abundance** of the unstable"* mutante, e poi asserisce causalità con *"directly correlated"*.
  - 🔴 **Non citare questo paper per alcun endpoint WOREE / SCAR12 / crisi:** `seizure` compare **zero volte** (parola in tondo). Genetica clinica su **n = 1**; tutto il lavoro funzionale è sovraespressione ingegnerizzata in CAL-62, BCPAP e HEK293T più xenograft in nude mouse; nessun knock-in, nessuna linea derivata dal paziente. `blind` e `random`: **zero** (in tondo, informativi); n = 3, media ± SEM, t-test bilaterali ripetuti senza correzione.
  - ✅ **Ciò che questa voce guadagna, in una frase:** la domanda diagnostica da porre all'allele missenso del genotipo di riferimento si affila da *«quanta proteina e quanta attività?»* a **«la proteina è presente ad abbondanza normale, oppure viene degradata nonostante un mRNA normale — e se sì, per quale via?»** Sono **due malattie molecolari diverse con punti di intervento diversi**, e ora si sa che la seconda è reale per una sostituzione missenso SDR di WWOX. **L'esperimento sopra va esteso con un braccio inibitore lisosomiale e un pulse-chase**, non riscritto.
...
- **Fonte**: **Obeid, Aqeilan et al. 2026, Mol Ther Adv vol 34** — supplemental S1–S8 (allegato) + main; predecessore proof-of-concept Repudi & Aqeilan 2021, EMBO Mol Med (PMID 34747138, [DOI](https://doi.org/10.15252/emmm.202114599)). Già a registro come [[paper_registry_current#PAPER 011]] + [[claim_registry_current#CLAIM 011]]
...
- 🔵 **Append-only, 2026-09-26 (`BATCH_20260926_ALDAZ`) — il «B» di questa mappa è più stretto nel modello che la voce propone come piattaforma.** Nel topo `Wwox^P47T/P47T` (PMID 36828035) la gliosi quantificata per immunoistochimica è **ippocampale** (nel cervelletto l'unica misura infiammatoria è l'mRNA di IL1β/TNFα, a un'unica età non dichiarata; la perdita di Purkinje è misurata a 80 e 250 giorni, senza confronto statistico fra le due età); l'astrogliosi cresce con l'età, la microglia peggiora con l'età solo nella **morfologia**, e se la sua **abbondanza** progredisca non è stato testato. Un esperimento che misuri Iba1 come readout di progressione misura quindi un asse che il paper di riferimento non ha mostrato progressivo. Vedi [[claim_registry_current#CLAIM 006]]. **Nessun altro contenuto di questa voce è modificato.**
...
- 🔵 **Append-only, 2026-09-26 (`BATCH_20260926_ALDAZ`) — la gamba cerebellare di questa voce è più stretta di come è scritta.** Nel topo `Wwox^P47T/P47T` la gliosi quantificata per immunoistochimica (conteggi e frazioni d'area Gfap⁺/Iba1⁺) è **ippocampale** (CA1, CA3, DG); nel cervelletto il paper misura la degenerazione delle cellule di Purkinje e un aumento ≥ 2 volte degli mRNA di IL1β e TNFα (qRT-PCR, n = 6 per gruppo, Fig. 9d–e, a un'unica età non dichiarata); per le Purkinje le due età sono misurate ma nessun confronto fra età è disegnato. L'astrogliosi cresce con l'età; per la microglia la progressione è mostrata **solo nella morfologia**, e la progressione dell'**abbondanza** non è stata testata (nessun confronto 80 vs 250 giorni), il che non equivale a un'assenza. Statistiche su subfield/cellule di n = 3 topi. Vedi [[claim_registry_current#CLAIM 006]]. **Nessun altro contenuto di questa voce è modificato.**
...
- 🔴 **AGGIORNAMENTO — 2026-09-21: l'arco è stato letto alla fonte per la prima volta, e il passo su cui poggia il razionale chetogenico è il più debole di tutti.** Letti i due paper che questo repository teneva come basi del metabolismo e che **nessuno aveva mai aperto** — erano gli **ultimi due `UNREAD_PREMISE` del repository**: `PMID 35328751` (primario) e `PMID 36271927` (review), Baryła *et al.*, **Medical University of Lodz, indipendenti da Chang/NCKU e da Aqeilan/HUJI**. Ricevute `FTR-20260921-35328751-01` e `-36271927-01`; audit [`wwox_metabolism_reviews_audit_20260921.md`](../analysis/wwox_metabolism_reviews_audit_20260921.md). Citazioni verificate personalmente contro gli artefatti.
  - 🔴 **Il collo di bottiglia PDK↑ ⊣ PDH — esattamente quello che la dieta chetogenica bypasserebbe — nelle cellule umane poggia SUL SOLO TRASCRITTO DI PDK.** Il braccio ossidativo **è stato misurato ed è nullo**: *"citrate synthase expression and activity were tested and **we did not observe WWOX influence on it**"*, e l'attività PDH resta invariata in **tutte e quattro** le condizioni. Nessun Seahorse, nessun OCR, nessun ECAR (parole in tondo, zeri informativi). **La metà «via dall'OXPHOS» della cornice Warburg, nell'uomo, non è misurata: è inferita da un mRNA.**
  - ⚠️ **E la direzione non è stabile: l'uptake di glucosio CAMBIA SEGNO con la condizione.** *"silencing significantly **increased** glucose uptake in normoxia normoglycemia and **reduced** glucose uptake in hypoxia hyperglycemia"*. Il lattato è l'unico readout coerente nelle quattro condizioni. Il sistema è **una sola linea immortalizzata di fibroblasti cutanei umani** (`1BR.3.N`), e **non è un knockout**: CRISPR policlonale con mRNA ridotto ~3×.
  - 🔴 **E la review, che è dello stesso gruppo sette mesi dopo, propaga una frase che i Results del proprio primario contraddicono.** Review: *"Additionally, it **increases the translocation of HIF1A to the nucleus**"*. Primario, Results § 2.6.1: *"We didn't observed WWOX influence on HIF1α protein in **nuclear fraction**"*, e la proteina HIF1α **citoplasmatica scende** (~2×, p<0.01). **Il blot era posizionato per vedere la traslocazione e non l'ha vista.** Quel che regge è il **reporter di transattivazione HRE** — il positivo robusto del paper; il livello proteico e la traslocazione nucleare no. È la **quinta** occorrenza odierna di re-voicing, e la forma peggiore: un laboratorio che cita la Discussion del proprio paper invece dei propri Results.
  - 🔴 **La sola affermazione direzionale della review sul metabolismo cerebrale va NELLA DIREZIONE OPPOSTA al razionale chetogenico:** *"It has been shown that **increasing glycolysis may slow neurodegeneration**."* La review non nota la tensione perché non sta scrivendo di questa malattia — ma questo modello sì, e deve portarla.
  - **Nulla di neurale e nulla di evolutivo nel primario:** `brain`, `neuron`, `epilep*`, `seizure` = **zero** (in tondo, informativi). La review tocca il cervello solo via **PCA di trascrittoma** (espressione, non flusso) su hNPC/neuroni, e lo dichiara: *"**suggested** … **most likely**"*. Il suo verdetto esplicito è: *"**The role of WWOX in the control of glucose metabolism in the context of neurological diseases requires further research.**"*
  - **Nessuno dei due propone un intervento metabolico:** `ketogenic`, `dichloroacetate`, `metformin` = **zero** in entrambi (in tondo, informativi). L'unica aspirazione terapeutica ha per bersaglio **WWOX stesso, nel diabete**.
  ...
  - ✅ **Che cosa NON cambia:** il meccanismo resta plausibile e l'esperimento già scritto qui sopra — **Seahorse OCR/ECAR su cellule donor-derived** — resta il falsificatore giusto, e **sale di priorità**, perché ora si sa che *nessuno l'ha mai fatto su WWOX in nessun sistema umano*. **Belief sul trasferimento al neurone: medio → BASSO.** ⚠️ La bibliografia di entrambi i paper è stata distrutta dall'estrattore (169 e 125 marcatori svuotati, **zero parentesi risolvibili**), quindi la tracciatura dei primari è **solo su ciò che la prosa nomina a parole** — debito dichiarato in `FT-111`.
...
- 🔴 **MISURATO 2026-09-21 — la domanda di questa voce non è mai stata posta a nessun allele WWOX, e ora si sa quanto è vuoto il campo.** Censimento vincolato (audit [`wwox_splice_transcript_census_20260921.md`](../analysis/wwox_splice_transcript_census_20260921.md); lettura `FTR-20260921-39101447-01`, che supera la ricostruzione legacy `FTR-20260726-39101447-01`).
  - **Su 20 record rilevanti: tre hanno MISURATO un trascritto da un allele di splicing WWOX, due lo hanno solo ANNOTATO** — sei pazienti su due alleli canonici a cui nessuno ha mai guardato l'RNA. **Uno solo dei tre è raggiungibile da qui.** I tre provengono da tre gruppi indipendenti fra loro e **tutti e tre fuori dal laboratorio Chang/NCKU**.
  - ✅ **Il dato che questa voce guadagna:** su **tutte e quattro** le misure del censimento **ogni prodotto riportato è uno skipping d'esone** — nessuno riporta ritenzione d'introne né sito criptico. ⚠️ Ma nell'unico paper leggibile lo strumento è un **minigene a due esoni con introne accorciato**, che è un cattivo strumento per *rilevare* le altre due possibilità: l'assenza di ritenzione e di sito criptico **non è esclusione**.
  - **L'analogo più vicino all'allele del genotipo di riferimento è stato misurato una volta e non è leggibile:** un accettore canonico `c.517-2A>G` → skipping dell'esone 6 (`PMID 30853297`). È **una frase di abstract su un paper irraggiungibile**: *riportato-come-misurato ma non letto*, censimento e non evidenza.
  - 🔴 **Nessun allele di splicing WWOX è mai stato saggiato con un inibitore di NMD** (`cycloheximide`, `emetine`, `actinomycin`, `nonsense-mediated` = zero, in tondo, informativi). **Quindi la frazione degradata contro stabile è ignota per ogni allele** — ed è esattamente la biforcazione che questa voce chiede di risolvere, già scritta nel suo esperimento.
  - 🔴 **Nessun rapporto aberrante:normale esiste da nessuna parte, e nessuna proteina è mai stata osservata da un allele di splicing WWOX.** L'esperimento già scritto qui sotto **non cambia e sale di priorità**: non è una conferma di ciò che altri hanno visto, **è la prima misura del suo genere su questo gene**.
  - ✅ **Lead tracciabile, come lead e non come piano:** il gruppo di Genova (`PMID 35573960`) **possiede fibroblasti WOREE ed esegue RT-PCR sui trascritti WWOX**; e **dieci pazienti su due alleli canonici** (`PMID 42721537`, cinque, `FT-110`; `PMID 26345274`, cinque, pacchetto `A8`) non hanno mai avuto l'RNA esaminato.
  - **Conseguenza su [[therapeutic_strategies_current#TX-001 — Correcting a canonical splice-acceptor allele: RNA assay → ASO/editing choice|TX-001]]:** il suo endpoint di efficacia **non ha un denominatore di base**, e **non esiste alcun braccio ASO o di correzione dello splicing in tutta la letteratura WWOX** (`antisense`, `oligonucleotide` = zero). Registrato là.
...
- 🔴 **AGGIORNAMENTO DI STATUS — 2026-09-21. Belief: medio → BASSO. Lo strumento resta; la lettura di malattia che ci era stata appesa no.** La fonte unica di questa voce (`PMID 25650666`, PMC4414138) è stata **riletta in chiave avversariale** — ricevuta `FTR-20260921-25650666-02`, `adversarial_reanalysis` sulla prima lettura `FTR-20260920-25650666-01`. Le citazioni qui sotto sono state **verificate personalmente contro l'artefatto**, non riportate da un delegato.
  - 🔴 **Il braccio umano post-mortem è un NULLO, e l'abstract lo presenta come un ordinamento temporale.** Didascalia della Figura 3, verbatim: *"**No significant difference** was shown in TPC6A or TIAF1 aggregation for samples between younger nondemented controls (14 samples per group; total 3 groups; **42 to 76 years old**) and older AD patients (24 samples per group; total 4 groups; **71 to 90 years old**)."* L'abstract lo rende come *"aggregate formation of TPC6AΔ **occurs preceding** Aβ generation in the hippocampi of middle-aged postmortem normal humans"* — una **affermazione temporale ricavata da un nullo trasversale**, su due fasce d'età **non sovrapposte**, in cui nessun soggetto è stato misurato due volte.
  - 🔴 **I controlli non sono age-matched: sono più giovani di ~21 anni** (59 ± 17 vs 80 ± 8.8). **Diagnosi ed età sono perfettamente confuse per ogni confronto fra gruppi di questo braccio**, inclusa la riduzione del p-WWOX su cui questa voce poggia. Nessun cieco (nessuna delle parole *blind* / *blinded* / *masked* compare — sono parole in tondo, quindi il conteggio è informativo), nessun PMI, nessun sesso, nessuno stadio di Braak, nessuna regola di scoring, nessuna correzione per confronti multipli.
  - **La riduzione del ~40% di p-WWOX esiste ed era riportata correttamente qui — ma eredita il confondimento e si autocita.** Verbatim: *"**In agreement with our previous reports**, the levels of protein aggregates for Tyr33-phosphorylated WWOX (p-WWOX) were significantly reduced by ~40% in the AD samples, compared to nondemented controls."* Stessa coorte, stesso salto d'età, stessi dot blot non ciechi. **Il contrasto è compatibile con l'invecchiamento quanto con la diagnosi**, e la sua unica corroborazione è il lavoro precedente dello stesso laboratorio.
  - 🔴 **Il filter retardation assay è cieco all'isoforma**, quindi l'attribuzione dell'abstract all'isoforma è un'inversione: *"**Both wild type TPC6A and TPC6AΔ are present in the aggregates**, as determined using the TPC6A (84–100) peptide antibody."* E la quantità che dovrebbe discriminare non discrimina: *"TPC6A aggregates were found in **both** control and AD groups **to a similar extent (~40% positive)**."* ⚠️ Attenzione a non confondere i due «~40%»: uno è la **frazione di campioni positivi, uguale nei due gruppi** (un nullo); l'altro è la **riduzione del p-WWOX** (un contrasto, confuso dall'età).
  - 🔴 **La «cascata sequenziale misurabile» non è mai stata misurata come sequenza.** Non esiste alcun saggio di legame nel paper: le stringhe *immunoprecipit*, *co-IP*, *pull-down* compaiono **zero volte** (parole di metodo in tondo, quindi il conteggio è informativo). E il paper si contraddice sul passo portante — la Discussion riassume *"We determined that TGF-β1 causes dissociation of WWOX from TPC6AΔ, thus leading to the aggregation … and Aβ production"*, mentre la **stessa Discussion** scrive *"Whether TGF-β1 regulates the binding of WWOX with TPC6AΔ **is unknown** and is being determined in this laboratory."*
  - **Il legame WWOX→aggregazione è reale ma è tutto ectopico, e il risultato endogeno più rilevante non ha figura.** Il braccio forte è *"When WWOX was knocked down by siRNA (WOX1si), ectopic TPC6AΔ and TIAF1 became aggregated by **greater than 80%** of cells (~100 cells counted)"* — in **COS7, fibroblasti di rene di scimmia**, a 24 h, scoring manuale, nessuna quantificazione dell'efficienza di knockdown, **nessun rescue per ri-espressione di WWOX**. L'unico enunciato endogeno di perdita di funzione è in Discussion e recita *"(**data not shown**)"*.
  - **Esistenza dell'isoforma: predetta, non osservata.** *"Results from 2 different web-based tools (NNSplice and NetGene) all **predicted** that the nt position 85 can be used as an alternative 5′ donor site"*; la proteina è *"deduced"*. Nessuna RT-PCR a cavallo della giunzione, nessun sequenziamento della giunzione, nessuna spettrometria di massa. **Gli antisieri sono validati con la sola competizione peptidica** — *"Specificity of the antisera was tested using the synthetic peptides to block immunostaining"* — cioè **senza alcun controllo negativo genetico**, benché il laboratorio usi un siRNA `TPC6Asi` **in questo stesso paper**. Per un policlonale su **neo-giunzione** è esattamente il controllo che manca.
  - 🔴 **Collasso di nodo, con conseguenza diretta su questa voce: TPC6AΔ e TIAF1 non sono due nodi che si corroborano, sono UNA misura.** Stesso laboratorio, **stessa banca di tessuti** (*"Department of Pathology, University of Colorado Health Sciences Center (by Dr. CI Sze, before 2005). IRB approval was waived."*), **stesse membrane di filter retardation**, stessa figura di sovraespressione, stessa pratica anticorpale — e *"Similar results were observed with TIAF1 aggregates."* **Peso di corroborazione della coppia: uno, non due.**
  - ✅ **Cosa resta, ed è la parte che vale:** il **pTyr33-WWOX come readout dello stato funzionale** rimane il candidato più vicino a una misura diretta, e le **sequenze peptidiche dei cinque anticorpi custom restano pubblicate e riusabili** — questo non è toccato da nulla di quanto sopra. Ciò che cade è l'uso del braccio umano come evidenza di malattia e la lettura della cascata come sequenza dimostrata. **La priorità dell'esperimento già scritto sotto sale**, perché la replica indipendente della biologia TPC6AΔ non è solo «mancante»: è l'unica cosa che potrebbe rendere questa voce utilizzabile.
  - **Esperimento più economico che ribalterebbe il verdetto:** materiale `TRAPPC6A` knockdown/knockout in cui la banda a 17 kDa e la colorazione delle placche **scompaiano**. Verificabile senza bancone: se il peptide di neo-giunzione TPC6AΔ compaia in una qualunque risorsa pubblica di proteomica MS su cervello umano.
  - **Limite dichiarato:** nessuna immagine di figura è ispezionabile in questo ambiente. Per `D-14`, il pannello di competizione peptidica e un eventuale contrasto wild-type/knockout per l'isoforma restano **non aggiudicati**, non scorati contro il paper.
...
- 🔴 **AGGIORNAMENTO DI STATUS — 2026-09-21. Belief: medio → BASSO. Status: open → maturing (piattaforma), con l'arco di sufficienza declassato ad asserzione degli autori.** Il paper è stato **letto integralmente per la prima volta** (ricevuta `FTR-20260921-25649963-01`, `partial_fulltext_read`, artefatto fingerprintato). Fino a oggi questo lead era **di seconda mano e senza ricevuta**. Cosa cambia, e nulla di ciò che segue è una smentita del paper — è una misura di quanto può portare:
  - **Il rescue è UNA frase non quantificata, senza comparatore.** Tutta l'evidenza: *"However, [wwox] mRNA rescued the phenotype in MO and siRNA injected embryos."* Nessun braccio full-length, nessun braccio WW-only, nessun mutante puntiforme, nessuna serie di dosi. La penetranza è riportata per il knockdown (56% / 62%) e **non** per il rescue. **Con un solo braccio la sufficienza non è testabile**, e la parola *sufficient* non compare nel paper.
  - **Il rescue è confuso dal disegno del costrutto.** La resistenza al reagente viene dalla **troncatura**, non da una ri-sintesi con mutazioni silenti: il frammento non lega MO né siRNA *perché è un frammento*. Resistenza al reagente e identità del dominio sono quindi **inseparabili**, e la lettura più parsimoniosa è che l'ADH sia stato scelto per comodità, non per testare la sufficienza.
  - **`D-04` morde qui.** Il costrutto che salva è un **SDR de-occluso**. In WWOX la stabilità si compra con l'occlusione e l'occlusione è anti-funzione: un frammento può funzionare **perché** è sfuggito ai domini WW. È un argomento sul frammento, non sul ruolo del dominio dentro la proteina intera. Va aggiunto che il costrutto **manca anche dell'NLS** (aa 50–55), non solo dei WW.
  - **Controlli di specificità, enumerati.** ASSENTI: secondo morpholino non sovrapposto; **morpholino p53** (il controllo standard per l'apoptosi off-target da MO — *p53* compare solo in una frase di omologia); verifica proteica del knockdown (nessun western, nessun anticorpo); mutante genetico; qualunque statistica sull'edema. PRESENTI: MO di controllo a 5 mismatch, e **un braccio siRNA indipendente**, che è la vera forza del lavoro. ⚠️ Mislabel nel testo: *"The standard MO control … was used as a **positive** control"* — un control MO standard è un controllo **negativo**.
  - **Il Ca²⁺ è un aggettivo, non un numero.** *"Patterns of Ca dynamics strikingly differed … with maximal Ca levels at the boundary between the edema and yolk"*; *"Relatively high rates"*. Nessun rapporto, nessun ΔR/R, nessuna traccia, nessun n, nessun p — da un sensore FRET ratiometrico costruito per dare numeri. Misurato **solo negli embrioni siRNA, solo a 30 hpf, solo DOPO la formazione dell'edema**, cioè a valle del cedimento della pompa cardiaca. Gli autori lo concedono: *"inhibition of [wwox] could have pointed a **non-cell-autonomous** phenotype."*
  - 🔴 **Il ponte verso le crisi si ritira.** La formulazione *"primo dato in vivo che lega la perdita di WWOX a una disregolazione del calcio, cioè un ponte meccanicistico verso le crisi"* è un **trasferimento di categoria**: quel calcio è peri-edema, cardiaco e intestinale, a 30 hpf, e **mai cerebrale**. L'unica osservazione CNS-adiacente di tutto il paper è *"weak localized expression … in optic nerves"*. Non c'è TUNEL, non c'è comportamento, non c'è saggio di crisi, non c'è alcuna misura neuronale.
  - **Correzione di denominatore:** `n=2460 / 2590 / 770` sono **uova iniettate**, non embrioni scorati — questo lead li usava come n di esito. La n della morfometria è separata e piccola: **30 embrioni in tre esperimenti indipendenti**.
  - **Sesta istanza abstract-vs-Results in questa letteratura:** l'abstract dice che il knockdown induce edema *"with similarities to conditions observed in human breast cancer"*, mentre la Discussion chiude con *"further studies are required to determine whether the present zebrafish edema was derived from carcinogenic processes."*
  - ✅ **Cosa resta, ed è la parte che vale:** **la piattaforma**, esattamente come questo lead l'aveva già identificata al punto (3). Morfante + lettura edema/morfometria + imaging Ca²⁺ ratiometrico + un braccio di rescue funzionante = un rig in vivo pronto in cui iniettare un allele umano della classe del genotipo di riferimento. L'esperimento già scritto sotto **non cambia e sale di priorità relativa**, perché è l'unico modo di far dire a questo sistema qualcosa che oggi non dice.
  - **Conseguenza su `TX-007` / `TX-003`:** **non citare questo lavoro in un argomento di packaging o di scope del costrutto.** La WWOX full-length sta già in un AAV; un dato ittico debole e senza comparatore non deve comprare una troncatura che scarta domini di necessità non testata — tanto più che la specie che salva **manca dell'NLS**.
  - **Statement causale, riscritto:** `wwox_loss —altera (qualitativamente, peri-edema, non-cerebrale)→ Ca²⁺_dynamics` regge come descrizione; `SDR_domain_alone —sufficient_to_rescue→ edema_phenotype` **è declassato ad asserzione non testata degli autori**.
...
- 🔴 **AGGIORNAMENTO — 2026-09-21: il gap terapeutico del corollario (c) È STATO COLMATO, E LE QUATTRO LEVE HANNO FALLITO TUTTE E QUATTRO, NELLO STESSO PAZIENTE.** Fonte: `PMID 35573960` (Riva A, Nobile G, … Zara F, Iacomino M 2022, *Front Pediatr* 10:847549, PMCID PMC9100683, Genova), **letto integralmente**; ricevuta `FTR-20260921-35573960-01`; audit [`woree_phenotypic_approach_audit_20260921.md`](../analysis/woree_phenotypic_approach_audit_20260921.md). Verbatim, verificato personalmente contro l'artefatto:
  > *"Several anti-seizure medications (ASM) were ineffective (i.e., valproate, **vigabatrin**, clonazepam, clobazam, levetiracetam, rufinamide, and **CBD oil**) or determined adverse events such as extreme drowsiness or increased secretions (i.e., phenobarbital, nitrazepam). Moreover, **three cycles of ACTH had very poor results**. **The ketogenic diet did not significantly improve seizures and was suspended.**"*
  - **Paziente (del case report pubblicato `PMID 35573960`):** biallelico null — uno stop
    `c.790C>T` (p.Arg264\*) e una delezione di 84 828 bp che rimuove l'esone 6, ciascuno da un
    genitore non affetto. **Due alleli null: nessuna proteina residua, nessun allele missenso.**
  - **Esordio il PRIMO GIORNO DI VITA**, contro una media WOREE dichiarata di 2–3 mesi: è la coda estrema precoce. Crisi toniche asimmetriche pluriquotidiane fino a 50/die, che *"never responded to any pharmacological treatment"*.
  - ⚠️ **Tensione interna non spiegata dal paper, segnalata e non risolta:** il vigabatrin è elencato fra gli inefficaci **ed è mantenuto in terapia** a 70 mg/kg/die.
  - ⚠️ **Il caso NON è una sindrome di West**, il che limita il confronto: `hypsarrhythmia`, `burst`, `spasms`, `West syndrome` compaiono **zero volte** (parole in tondo, zeri informativi). Non è quindi comparabile agli spasmi infantili vigabatrin-responsivi riportati altrove.
  - ⚠️ **Nota di denominazione, non un negativo:** la stringa `cannabidiol` compare zero volte **perché il paper scrive «CBD oil»**.
  - **Che cosa cambia e che cosa no.** `n = 1`, un genotipo null/null, un esordio atipicamente precoce: **non è una confutazione delle quattro leve nella WWOX-DEE**, ed è sbagliato leggerlo così. Ma il corollario (c) diceva *«la letteratura sul null non ha mai testato le leve che a noi interessano»*: **ora le ha testate, una volta, e nessuna ha funzionato.** La formulazione corretta d'ora in avanti è: *le quattro leve sono state provate in un paziente WOREE biallelico-null a esordio primo-giorno e nessuna ha migliorato le crisi; nessuna è stata provata in un genotipo con proteina residua.*
  - 🔴 **E un secondo reperto che tocca `CLAIM 039` e va nella direzione opposta al risparmio cerebellare** — **in prosa**, quindi `D-14` non lo scherma: *"a small inferior vermis"* già a **7 giorni**; *"Mild signal alterations were also noted at the level of the pons and **dentate nuclei**"* a 2a4m; *"inferior cerebellar vermis hypoplasia"* in Discussion; e a livello di letteratura *"cerebellar vermis hypoplasia … **have been described in most cases**."* ⚠️ **Non è però lo stesso enunciato** dell'istologia cerebellare indenne del ratto `lde/lde`: **ipoplasia vermiana mediana alla RM e corteccia cerebellare indenne al microscopio ottico possono essere entrambe vere.** `thalam*` = **zero** in questo paziente, il che è da registrare perché `26345274` riporta degenerazione talamica mediodorsale selettiva in uno dei suoi cinque.
  - 🔴 **Lezione metodologica con conseguenze su ogni censimento di classe allelica:** il secondo allele — una delezione intragenica di un singolo esone — **è sfuggito al CNV calling su esoma** in un centro esperto (*"CNVs analysis was also unremarkable"*) ed è stato trovato solo con Q-PCR e confermato con array-CGH. **I censimenti di classe allelica costruiti su pazienti diagnosticati in NGS sono limiti inferiori per le CNV intrageniche.**
  - ✅ **Capacità sperimentale da annotare:** questo gruppo **possiede fibroblasti del paziente e vi esegue RT-PCR sui trascritti WWOX** — esattamente la classe di saggio che [[discovery_ledger_current#DL-BIO-002 — Trascritto/proteina dell'allele di sito accettore, nominati dalla predizione di skipping esone 9 (SpliceAI/Pangolin)|DL-BIO-002]] nomina come mancante per l'allele di sito accettore.
...
- 🔴 **AGGIORNAMENTO DI STATUS — 2026-09-21. Status: open → `refuted as a restoration lead`. Tag: ESPANSIONE → ESPANSIONE (chiusa in direzione negativa).** Questa voce era rimasta `open` con l'istruzione «recuperare il full-text di Lo 2008 … se il peptide è attivo per via periferica, diventa un lead serio», mentre [[therapeutic_hypotheses_ledger_current#HYP-20260709-04 — Peptide pTyr33-WWOX come rescue funzionale a valle della variante|HYP-20260709-04]] — che da questa voce dipende — era già stata portata a **`refuted`** il 2026-07-09b. **Omissione di propagazione: il ledger terapeutico sapeva, il discovery ledger continuava a indicare un lead da inseguire.** Chiusa qui.
  - **Il segno è invertito, e lo dice il primario stesso.** Dall'abstract di Lo CP, Hsu L-J, Li M-Y, Hsu S-Y, Chuang J-I, Tsai M-S, Lin S-R, Chang N-S, Chen S-T 2008, *Eur J Neurosci* 27(7):1634–46 (PMID 18371080, [DOI](https://doi.org/10.1111/j.1460-9568.2008.06139.x), recuperato via PubMed): *"activated WOX1 plays an **essential role in the MPP+-induced neuronal death**"*, e *"Dominant-negative WOX1, a potent inhibitor of Tyr33 phosphorylation, **abolished** this event"*. Nel sistema in cui il peptide è stato testato, **l'attivazione di WWOX è il segnale di morte**: il peptide protegge **bloccando** una via WWOX-dipendente, non sostituendola. In un genotipo **privo di proteina WWOX funzionale non esiste il segnale pTyr33 da bloccare**, e inibire una funzione assente non ripristina nulla.
  - **La citazione di Sze 2015 è fedele; è l'interpretazione del commentary a non reggere.** Verificata verbatim nell'artefatto `files/fulltext/PMID26355344_PMC_MCPtext.txt`: *"A small Tyr33-phosphorylated WWOX peptide, which mitigates MPP+-mediated Parkinson-like syndrome in rats, may be of therapeutic use in the restoration of neural function under WWOX deficiency."* **`D-15` si applica al contenuto, non alla trascrizione:** LEGEND aveva riportato correttamente ciò che il commentary dice; il commentary dice qualcosa che il suo primario non sostiene. *(Nota di strumento: l'artefatto rende la frase senza il conclusivo* in vivo *presente nella citazione originale — è la nota cancellazione dei token in corsivo da parte dell'estrattore, non una discrepanza di fonte.)*
  - **Indipendenza, mai registrata prima in questa voce:** Lo 2008 **è un lavoro del laboratorio Chang/NCKU** (Chang N-S ottavo autore, Hsu L-J seconda, affiliazione National Cheng Kung University). Primario e commentary appartengono quindi **allo stesso gruppo**: il commentary non è una lettura esterna che corrobora il primario, è lo stesso laboratorio che ri-presenta il proprio dato in una cornice terapeutica più forte di quella che il dato regge.
  - **Stato di acquisizione:** full-text **non ottenibile in questo ambiente** — nessun PMCID (verificato con `convert_article_ids`: il record restituisce solo il PMID), e il full-text Wiley è paywalled. Va nel pacchetto di acquisizione umana, **non** nella coda di lettura automatica. La voce FT annunciata qui come «FT da creare» non è mai stata creata e **non va creata come target di lettura**: è un debito di acquisizione.
  - ✅ **Cosa resta, ed è diverso dall'ipotesi originale:** il peptide potrebbe valere come **neuroprotettore generico**, perché blocca una via di morte neuronale a valle. È un meccanismo distinto, mai testato in WWOX-deficiency, e **non eredita il razionale di "restoration"**. Non presentarlo come opzione: non c'è dose, via, penetrazione BBB né durata, in nessuna delle due fonti.
  - **Nota d'onestà, invariata rispetto a HYP-20260709-04:** questa chiusura poggia sull'**abstract** del primario, non su una lettura integrale. L'abstract è esplicito e autonomo sul punto di segno, ma un abstract non è una lettura. **Se il full-text mostrasse altro, la voce va riaperta.**
...
- 🔵 **Append-only, 2026-09-22 (Orchestrator) — l'esperimento in-silico che questa voce chiama *"il prossimo passo in-silico prioritario"* È STATO ESEGUITO.** Il risultato è in [[discovery_ledger_current#⭐ DL-MECH-037 — Q230P è **l'unica delle tre varianti a essere una lesione di ripiegamento del core**. Il ΔΔG non lo diceva; la sepoltura sì.|DL-MECH-037]], 87 righe più sotto in questo stesso file, e **nessuno dei due rami previsti qui vince pulito**: G372 è superficiale (relSASA 0.163, ramo 1) **ma** destabilizzante quanto Q230P (ΔΔG +1.58 vs +1.51, ramo 2). A discriminare non è il ΔΔG — è la **sepoltura**. Il controllo decisivo non era G372R ma **P47T/P47R**: stesso residuo, ΔΔG quasi identici, fenotipi opposti. Puntatore aggiunto perché un lettore che atterra qui viene informato che l'esperimento è pendente, e l'esito non era raggiungibile da questa voce. **Nessun contenuto di questa voce è modificato.**
- 🔵 **Append-only, 2026-09-26 (`BATCH_20260926_MALLARET`) — la riga P47T è più forte della sua fonte.** In Mallaret 2014 (PMID 24369382) l'abbondanza della proteina P47T è giudicata *"based on visual inspection"* nei fibroblasti di un solo paziente, senza densitometria: è **presente e simile ai controlli a occhio, non quantificata** — non "a livelli normali" come misura. Il binding perduto è **in vitro, verso un solo peptide PPPY** (WBP1); la generalizzazione ai motivi PPxY è della didascalia degli autori. La conclusione di questa voce non dipende da quella parola; vedi [[claim_registry_current#CLAIM 030]]. **Nessun altro contenuto è modificato.**
...
- 🔵 **Append-only, 2026-09-26 (`BATCH_20260926_MALLARET`) — le parole su P47T in questa voce sono più forti della fonte.** In Mallaret 2014 (PMID 24369382) la proteina P47T è *presente, di abbondanza simile ai controlli all'ispezione visiva (non quantificata)*, in un solo paziente e senza densitometria — non "a livelli normali" o "a livello wild type" come misura; il binding è perduto *in vitro* verso un solo peptide PPPY (WBP1), e la generalizzazione ai motivi PPxY è della didascalia degli autori. Vedi [[claim_registry_current#CLAIM 030]]. **Nessun altro contenuto di questa voce è modificato.**
...
- 🔵 **Append-only, 2026-09-26 (`BATCH_20260926_ALDAZ`) — la prima riga della tabella è più forte della fonte primaria.** «Proteina pari al wild type» per `Wwox^P47T/P47T` è la parola della review (PMID 42128308); nel primario (PMID 36828035, lettura completa `FTR-20260913-36828035-03`) gli autori scrivono «similar levels» sulla corsia input 10% del pull-down, n = 2 per genotipo, **non quantificata in nessun punto del paper**: la banda Wwox è presente in tutte e quattro le corsie, e l'abbondanza di proteina **non è mostrata né invariata né ridotta**. Ciò che è quantificato è il **trascritto**, invariato in cervelletto (qRT-PCR, n = 6) e in quattro regioni (RNA-seq, n = 5). La tesi della voce — l'abbondanza non legge la funzione — **regge sul braccio del trascritto**, non su quello della proteina. Nella stessa riga, «neuroinfiammazione progressiva» vale per l'astrogliosi ippocampale; per la microglia la progressione è mostrata solo nella morfologia ([[claim_registry_current#CLAIM 006]]). Vedi [[claim_registry_current#CLAIM 007]]. **Nessun altro contenuto di questa voce è modificato.**
- 🔵 **Append-only, 2026-09-26 (`BATCH_20260926_MALLARET`) — le parole su P47T in questa voce sono più forti della fonte.** In Mallaret 2014 (PMID 24369382) la proteina P47T è *presente, di abbondanza simile ai controlli all'ispezione visiva (non quantificata)*, in un solo paziente e senza densitometria — non "a livelli normali" o "a livello wild type" come misura; il binding è perduto *in vitro* verso un solo peptide PPPY (WBP1), e la generalizzazione ai motivi PPxY è della didascalia degli autori. Vedi [[claim_registry_current#CLAIM 030]]. **Nessun altro contenuto di questa voce è modificato.**
...
### DL-METH-097 — **In nove superfici su trentaquattro le didascalie stanno FUORI dal `<body>`, e chi legge col `<body>` non le vede mai**
...
*(Rinumerata `092 → 097` alla fusione dello stesso giorno, per collisione con `DL-MECH-092`
di un altro ramo. Le due modifiche sono arrivate sullo stesso titolo da due lati e sono state
tenute entrambe: il numero da qui, il conteggio corretto dall'altro. **Scegliere un lato
avrebbe silenziosamente disfatto una correzione**, che è il modo in cui una fusione perde
lavoro senza che nessuno se ne accorga.)*
...
> **Nota di merge, 2026-08-11 (quinto blocco).** La voce che segue arriva da un altro ramo e
> alloca `DL-METH-093`. **Non collide** con `DL-META-093` qui sopra: la numerazione di questo
> ledger è **per famiglia** — `DL-BIO-001`, `DL-MECH-001`, `DL-MOL-001` e `DL-REPO-001`
> convivono dall'inizio — e `METH` e `META` sono famiglie diverse. Nessuna rinumerazione.
>
> Vale ricordarlo perché tre note di merge precedenti in questo file avevano asserito il
> contrario, e su quella premessa falsa erano state fatte rinumerazioni non necessarie.
...
- **Destinazione:** commit candidate `CC-20260811-WWOX-MOUSE-SERIES-01`.
### DL-METH-094 — **L'autorialità non è condizione sufficiente dell'erosione: la prova è interna a un solo paper. Sulla figura-modello resta un'ipotesi replicata due volte**

> 🔴 **Titolo corretto il 2026-08-11**, da *«Non sono gli autori a perdere l'attenuazione: è la figura-modello»*. Quel titolo affermava due cose che i dati non reggono: che l'autorialità **non abbia** valore predittivo — mentre la lettura mostra soltanto che **non basta** — e che la figura-modello sia **ciò che predice** il fenomeno, mentre due figure di **un solo laboratorio** fanno un'ipotesi replicata due volte, non una causa stabilita. È la lezione di `DL-METH-092` applicata a se stessa: **a viaggiare è il titolo**, e questo era più forte del corpo.

- **Classificazione:** `DATO` sul confronto, verificato su entrambi i lati contro un unico artefatto impronta-verificato; `INFERENZA` sulla generalizzazione oltre questo laboratorio.
- **Fonte:** `PMID 31075076` — Abdeen & Aqeilan, *Cell Cycle* 2019;18(11):1177–1186, review del proprio studio primario. `FTR-20260811-31075076-01`, `file:files/fulltext/PMID31075076_Abdeen2019_PMC.html @ 556336ea…`
- 🔴 **FALSIFICA LA FORMA UNIVERSALE DI `DL-METH-091`, ED È IL MOTIVO PER CUI QUESTA LETTURA VALEVA.** Avevo concluso che *«il commentary di un autore sul proprio paper non è un riassunto: la modalità si degrada»*. Questa è una **review dello stesso laboratorio sul proprio studio primario**, e il testo corrente **attenua correttamente e ripetutamente**: *«Our recent study **suggests** new mechanisms»* · *«The first mechanism **we propose**»* · *«it is **possible to speculate**»* · *«is **still to be investigated**»*. **L'autorialità non è dunque condizione *sufficiente* dell'erosione.** 🔴 Correzione del 2026-08-11: avevo scritto *«l'autorialità non predice l'erosione»*, che è più forte di quanto un controesempio possa dimostrare. **Un controesempio abbatte l'universale, non il valore predittivo**, e `DL-METH-091` resta in piedi nella sua forma condizionata.
- 🔴 **DOVE I MODALI SI COMPRIMONO — e il confronto è INTERNO.** Il paragrafo che introduce la Figura 1 attenua **tre volte in due frasi**. La didascalia immediatamente sotto enuncia gli stessi tre meccanismi **all'indicativo**: *«WWOX modulates ER expression and function, which in turn positively or negatively affects p53»*, senza alcun modale. E il disegno non porta né tratteggio né punto interrogativo. Stessi autori, stesso anno, stessa rivista, **stesso paragrafo**: controlla ciò che servirebbe a una spiegazione basata su chi scrive o su dove. **La compressione non è fra documenti: è fra il testo corrente e il diagramma dentro un solo documento.**
  🔴 **Ma non è vero che la figura non si qualifichi affatto** — correzione del 2026-08-11. La didascalia **apre** con *«WWOX regulates p53 function, **model** of action and scenarios»*: **la parola «model» è stampata.** L'affermazione vera e più stretta è che la didascalia etichetta l'insieme come modello **mentre enuncia ogni singolo meccanismo all'indicativo**, e che le tre attenuazioni specifiche del testo adiacente non sopravvivono in essa. La versione precedente diceva «nessuna qualificazione», ed era falsa contro il pixel e contro la prosa.
  🔴 **Corretto anche il conteggio dei nodi:** sono **cinque**, non sei — WWOX, Genomic instability, ER pathway, EMT, p53. E la topologia: `ER→p53` due linee opposte; `Genomic instability↔p53` due inibizioni reciproche; **`p53–EMT` una sola linea inibitoria, non due reciproche.** Il locator originale descriveva pixel inesistenti.
- **La seconda metà replica `PMID 27308504`, e la formulazione va ristretta.** L'unico punto in cui *questa* figura marca è `ER→p53`, disegnato con **due linee opposte simultanee** — ed è il punto in cui **il testo stesso conserva esplicitamente incertezza direzionale** (*«positively or negatively»*). Su `27308504` l'unica attenuazione era un `?` sul complesso MRN, che il testo di quel paper non rivendica. 🔴 **La versione precedente diceva «dove gli autori non hanno interesse»: è attribuzione psicologica non misurata, ed è ritirata.** Due paper di **un solo laboratorio**: **ipotesi replicata due volte**, non regola stabilita sulle figure-modello in generale.
- **Conseguenza operativa, e sostituisce quella di `DL-METH-091`:** non «diffidare dei commentary d'autore», che è falso, ma **`PREMISE_TAG` obbligatorio su ogni affermazione tratta da una figura-modello**, indipendentemente da tipo di articolo, rivista e autore. E dove il testo corrente accanto alla figura porta un'attenuazione che la didascalia non porta, **è il testo corrente a fare fede**.
- **REVIVAL_TRIGGER / falsificazione:** una figura-modello, in un paper che attenua nel testo, che porti attenuazione anche nel disegno — tratteggio, `?`, o «proposed» nella grafica — su una rivendicazione **propria** degli autori. Due istanze contrarie basterebbero a rovesciare la regola.
- **Interconnessioni:** `DL-METH-091` (**forma forte falsificata, non cancellata**) · `DL-METH-093` · `IMPORTED_PREMISE_ATTRIBUTION_GATE` · [[epistemic_discipline]] · `FT-070`.

#### Due reperti che viaggiano con questa voce

**Un fenotipo eterozigote, registrato come EVIDENZA SECONDARIA** — 🔴 corretto il 2026-08-11 su due punti. *«aged germline Wwox-heterozygous mice in mammary susceptible C3H genetic background develop mammary tumors with ~50% penetrance»*: questa review **non lo misura, lo riporta**, citando il proprio rif. 40 — Abdeen SK, Salah Z, Maly B, *et al.*, *Wwox inactivation enhances mammary tumorigenesis*, **Oncogene 2011;30(36):3900–3906**, non letto. Avevo scritto «esiste ed è misurato», accreditando una misura a un documento che non ne contiene. E avevo scritto **«il primo fenotipo eterozigote che questo corpus àncora»**: un'affermazione sull'intero corpus fatta **senza averlo verificato**, ritirata invece che difesa — richiederebbe un audit. **Quel che resta valido è il contrasto** con `27308504`: là uno schema stampava `WWOX+/−` mentre nel primario che riformulava **nessun sistema eterozigote esisteva**; qui un sistema eterozigote almeno **esiste nella letteratura citata**. E le condizioni al contorno pesano quanto il numero: tumori **mammari**, ceppo **scelto per suscettibilità mammaria**, animali **anziani**. Un fenotipo oncologico in un fondo sensibilizzato **non è** un'affermazione sui portatori di un genotipo neurosviluppo.

**Una POSSIBILE terza direzione, non letta, su un conflitto già aperto** — 🔴 corretto: non «una terza voce» già stabilita. Qui si **riporta**, in una frase, che Schrock MS, Batar B, Lee J, *et al.*, *Wwox-Brca1 interaction: role in DNA repair pathway choice*, **Oncogene 2017;36(16):2215–2227** (`PMID 27869163`, `PMC5398941`) mostri il complesso BRCA1–WWOX sostenere il **NHEJ**. `DL-METH-084` porta già il conflitto HDR/NHEJ fra `25331887` e `38499540`. **Ciò che questo corpus possiede è il riassunto che una review fa di quel paper, lungo una frase**: se Schrock stabilisca la dominanza del NHEJ, e a quali condizioni, è ignoto finché non è letto. Chiamarlo terza direzione stabilita sarebbe **esattamente il difetto di premessa importata** che questa lettura documenta. Accodato in `FT-070`.

---

### DL-BIO-095 — **Il primo fenotipo umano quantitativo di WWOX fuori dal cancro e fuori dal neurosviluppo — ed è dominante, piccolo, e non codificante**

- **Classificazione:** `DATO` sull'associazione e sui numeri; `DATO` sulla funzione in vitro con il limite dichiarato dagli autori; `ESPANSIONE` su qualunque trasferimento al genotipo di riferimento.
- **Fonte:** `PMID 18674750` — Lee JC, …, Pajukanta P, *Am J Hum Genet* 2008;83(2):180–192. `FTR-20260811-18674750-01`, `file:files/fulltext/PMID18674750_Lee2008_PMC.html @ 7c12f31f…`. Lettura **parziale**: corpo integrale, 2 tabelle, **3 figure su 3** ispezionate, **supplementary non recuperato**.
- **Il reperto.** `rs2548861`, **introne 8** di `WWOX`, associato a HDL-C basso con significatività region-wide (`p = 6,9 × 10⁻⁷`) su **9 798 soggetti** — famiglie dislipidemiche, casi-controllo, e due coorti finlandesi *non selezionate*, di cui una **prospettica su 21 anni**. Reporter luciferasi ed EMSA in HepG2 mostrano che la regione funziona da **elemento cis-regolatorio con effetto allele-specifico**.
- 🔴 **Perché tocca questo modello, e con quale cautela.** L'effetto è **DOMINANTE**: *«one copy of the T allele … increases the probability for low HDL-C by approximately 17%»*. Questo corpus ha discusso a lungo che cosa faccia un eterozigote WWOX — su `27308504` uno schema stampava `WWOX+/−` su evidenza che **non conteneva alcun sistema eterozigote**. Qui l'effetto eterozigote **è la misura**. **Ma è piccolo e gli autori lo dichiarano**: *«not a major HDL-C determinant but rather … a modifier variant»*, **1,5%** della varianza. Un allele regolatorio dominante e di piccolo effetto **non è** una perdita di funzione biallelica: l'unico trasferimento onesto è a livello di **domanda** — *il dosaggio di WWOX conta fuori dal sistema nervoso, nell'uomo, su scala di popolazione?* — e la risposta qui è **sì**.
- 🔴 **Una classe di meccanismo nuova per questo corpus: il non codificante.** Il risequenziamento degli esoni trova **un solo** non sinonimo (Ala179Val) e **non è associato** (`p = 0,63`). L'introne 8 porta inoltre un **secondo enhancer funzionale**, dimostrato *in vivo* in topi transgenici da terzi, e tre delezioni omozigoti in linee di adenocarcinoma gastrico. **Due elementi funzionali e due segnali di associazione indipendenti in un solo introne**: l'architettura regolatoria di `WWOX` è un soggetto a sé, e qui ogni meccanismo letto finora è a livello proteico. `FT-071`.
- **Il negativo, messo dagli autori stessi:** `rs2548861` **non** è significativo nella meta-analisi GWAS sui lipidi con modello additivo. La loro spiegazione — accertamento diverso, dominante contro additivo, diabete e obesità confondenti — è ragionata e plausibile, e **il fatto viaggia col risultato**. E il top hit GWAS della regione, `rs2667590`, sta **nello stesso introne 8 senza alcun LD** (`r² < 0,02`): due varianti indipendenti in un introne sostengono **il locus**, non la variante.
- **Due reperti dai pannelli, che nessuna frase del testo contiene.** *Figura 2*, la rivendicazione longitudinale: **asse x non in scala** — 1980, 1983, 1986, 2001 equispaziati, quindi 15 anni occupano la larghezza di 3, e **tre dei quattro punti stanno nei primi sei anni**; e **nessuna barra d'errore** su otto punti, mentre la Tabella 2 li riporta tutti come media ± SEM. Le due tracce sono **parallele**: il risultato è **persistenza, non progressione**. *Figura 1*: il picco è **isolato** — il punto più alto successivo in 12,4 Mb sta a ~3,1, e fra 3,1 e 5,85 non c'è nulla, benché il testo dichiari LD con i tag SNP immediatamente circostanti.
- **REVIVAL_TRIGGER / che cosa lo rafforzerebbe:** il supplementary S4 (EMSA competitiva, l'unico supporto quantitativo del legame allele-preferenziale); la lettura del rif. 42 (secondo enhancer in introne 8); e un dato di espressione di WWOX allele-specifico in fegato umano.
- **Conseguenza operativa:** nessun claim canonico può usare questo lavoro per affermare qualcosa sul fenotipo neurologico. Vale per la domanda *«WWOX ha funzioni tessuto-specifiche misurabili nell'uomo?»* e come **primo ancoraggio umano di un effetto eterozigote**, con i suoi limiti attaccati.
- **Interconnessioni:** `FT-057` (il KO murino, di cui questo paper riporta di seconda mano la morte metabolica inspiegata) · `FT-071` · `DL-METH-094` · [[gold_is_in_the_details]].
...
### DL-METH-098 — **Non solo la modalità: anche l'AMPIEZZA. Un titolo di figura generalizza a «in osteosarcoma» ciò che il paper stesso dichiara non evidente nell'umano**
...
- **Interconnessioni:** `DL-METH-094` (**sorella, non identica**) · `DL-METH-091` · `FT-072` · [[epistemic_discipline]].
...
### DL-THER-105 — **Il rescue di WWOX non è monotòno su tutti gli endpoint: normalizza due marcatori di strato corticale su tre e SUPERA il wild type sul terzo**

- **Classificazione:** `DATO` sul pannello e sulla premessa statistica, entrambi ancorati su artefatti impronta-verificati · `INFERENZA` sulla selettività per marcatore · `IPOTESI` sulla non-monotonicità della dose.
- **Fonte:** `PMID 34268881` — Steinberg et al. 2021, *EMBO Mol Med*. `FTR-20260909-34268881-04`, `file:files/fulltext/PMID34268881_assets/EMMM-13-e13610-g008.jpg` (Figura 4H) e `file:files/fulltext/PMID34268881_Steinberg2021_PMC.xml @ de340289…`. Locators `entries[12..14]` e `entries[17]`.
- **Il reperto, letto dai pixel a 400 dpi.** Figura 4H quantifica tre marcatori di strato corticale in WT, WWOX-KO e rescue AAVS1 (`W-AAV`, promotore UBP costitutivo). Per **CTIP2/nuclei** la barra WT-vs-W-AAV **è tracciata ed è significativa (`**`), con il rescue SOPRA il wild type**. Per **TBR1** e per il terzo pannello (**SATB2**) nessuna barra WT-vs-W-AAV è tracciata: poiché la didascalia dichiara *«one‐way ANOVA with Tukey's multiple comparisons test»*, ovvero **tutte le coppie per costruzione**, l'assenza significa **testato e non significativo**. Il rescue quindi **normalizza due marcatori e ne scavalca uno**.
- **Che cosa dice il testo, ed è corretto ma incompleto.** *«…recovered and layering improved in W‐AAV COs **compared with WWOX‐KO COs**, RNA levels did only partially»*. Il confronto è esplicitamente contro il KO. Dove il confronto contro il WT **è** tracciato, va nella direzione opposta.
- 🔴 **La prima stesura di questo reperto era un OVERSHOOT, corretto dall'audit cieco prima di arrivare qui.** Diceva che per TBR1 e SATB2 il confronto col WT era *«non testato»*. Il test è all-pairwise e il pannello CTIP2 dimostra che gli autori la barra la tracciano quando è significativa. **`Non tracciato` non è `non eseguito`, e non è `non significativo`, e le tre cose vanno tenute distinte**: qui è la seconda. La premessa che regge l'inferenza è ora ancorata come `entries[17]` invece di restare ragionamento di sfondo.
- **Perché conta per il portafoglio terapeutico.** `CLAIM 032` fissa la soglia **inferiore** («l'aploinsufficienza non è deleteria: basta correggere un allele»). Questo reperto è la prima misura in questo corpus che tocca il **lato opposto**: un'espressione sovrafisiologica e ubiquitaria non riporta ogni endpoint al set point, su almeno un marcatore di patterning lo supera. Le due si compongono in una **finestra**, non in un pavimento.
- **Che cosa NON è.** Non è un reperto di tossicità: nessun endpoint qui misura danno, e uno scavalcamento di marcatore non è un fenotipo. Non tocca l'elettrofisiologia — il firing **è** normalizzato (`P = 0.7681`, genitori vs organoidi paziente rescued), e l'audit è stato esplicito che nulla in questa serie riguarda firing o Ca²⁺.
- **`PREMISE: DEFAULT_FROM_TEXTBOOK`** sull'assunto mai scritto che *più WWOX ripristinata sia monotonicamente meglio*. È un bersaglio di ricerca, non una fondazione.
- **REVIVAL_TRIGGER:** una serie di rescue **titolata**, con il comparatore wild type tracciato **a ogni dose** e almeno un endpoint di patterning corticale. Falsificherebbe la non-monotonicità, o la misurerebbe.
- **Interconnessioni:** [[claim_registry_current#CLAIM 002]] (candidato `CC-20260909-34268881-01`) · [[claim_registry_current#CLAIM 032]] (soglia inferiore — questa è la superiore) · [[claim_registry_current#CLAIM 011]] (soglia contro continuum dose-risposta, stessa forma di errore) · `PMID 42422765` Fig S8 (confronto mai tracciato).

#### Due negativi adiacenti, ancorati perché nessuno li aveva ancorati

`Appendix Table S1` **è** distribuita anche se le Appendix Figs S1–S6 non lo sono, e porta i P-value esatti di quattro delle sei figure mancanti. Due valgono da soli: la **dimensione degli organoidi è un nullo pulito** a tutti e quattro i tempi (wk6 `0,3675` · wk10 `0,5619` · wk15 `0,9949` · wk18 `0,8952`), quindi il modello **non riproduce la microcefalia** di parte dei pazienti — e questo è un test **eseguito e riportato**, non una barra assente; e l'identità dorso-ventrale è invariata ma **non uniformemente**, con il set ventrale a `0,064` contro `1` per dorsale e combinato, dove il testo corrente riporta soltanto «no statistically significant differences».

---

### DL-BIOM-014 — **Il set di geni glicolitici che riporta la perdita di WWOX non è fisso: cambia fra due contesti dello stesso tessuto, e il gene di punta è quello che cade**

- **Classificazione:** `DATO` sui pannelli, ancorati su artefatti impronta-verificati · `INFERENZA` sulla dipendenza dal contesto · `IPOTESI` sull'implicazione per un biomarcatore trascrizionale.
- **Fonte:** `PMID 29724996` — Abu-Remaileh et al. 2018, *Cell Death Dis* 9:511. Ricevuta `FTR-20260909-29724996-01` (`complete_fulltext_read`). Locators `entries[30]`, `entries[33]`, `entries[28]`. `file:files/figures/PMID29724996/SuppFig4_p7-7.png @ 458fad12…` e `file:files/fulltext/PMID29724996_AbuRemaileh2018_supplement.txt @ 3882f4aa…`.
- **Il reperto.** Nel modello DEN il bersaglio di punta è **PKM2**: enrichment ChIP più alto (Fig 4f), unico con conferma proteica IHC (Fig 4e). Sotto **dieta ad alto contenuto lipidico a 11 mesi** (`Supplement S4C`) **PKM2 non è elevato** — sta sotto il controllo, non marcato, direzione invertita — mentre sono marcati **PDK1, LDHA1, ENO1**. Nel cohort **DEN-free** (`S3A`) i marcati sono ancora diversi: **PKM2 e PDK1**, con LDHA1/HK2/GLUT1 nulli.
- **Il confine che nessuno aveva ancorato.** `Supplement Table 2` chiude la batteria ChIP a **cinque promotori** — PKM, GAPDH, HK2, Glut1, Aldoa — più due controlli di regione negativa. **PDK1 e LDHA1 non sono mai testati in ChIP.** L'occupancy promotoriale, cioè la parte meccanicistica del claim, **non copre** i due bersagli che in questo repository portano i ponti cross-paper più forti.
- **Perché conta come lead di biomarcatore.** `CLAIM 025` propone il rapporto WWOX/HIF1A come marcatore di stato sistemico. Questo reperto non lo smentisce: lo **vincola**. Un marcatore trascrizionale costruito su un singolo transcript glicolitico — e PKM2 è la scelta ovvia, essendo il gene di punta — **fallirebbe nel contesto lipidico**, che è precisamente il contesto metabolico più vicino all'asse WWOX–HDL/lipidi già presente nel modello (`PAPER 062`, `PMID 24871327`). Un pannello multi-gene con direzione dichiarata per contesto è la forma difendibile; un singolo gene no.
- **Che cosa NON è.** Non è una misura di flusso: il paper misura mRNA, proteina per IHC e occupancy, mai respirazione o flusso glicolitico. Non è un dato neurale: nessun endpoint del paper è neurologico.
- **`PREMISE: DEFAULT_FROM_TEXTBOOK`** sull'assunto mai scritto che *«i target di HIF1α» siano un insieme stabile* che si muove in blocco. È un bersaglio di ricerca, non una fondazione — e qui è già falsificato entro un solo tessuto e una sola specie.
- **REVIVAL_TRIGGER:** una misura di occupancy HIF1α su promotori *Pdk1* e *Ldha* in tessuto WWOX-deficiente, oppure un pannello glicolitico misurato negli stessi animali in due contesti metabolici con il comparatore tracciato in entrambi. Chiuderebbe il confine 2.1 o lo estenderebbe.
- **Interconnessioni:** [[claim_registry_current#CLAIM 025]] (candidato `CC-20260909-29724996-01`) · [[claim_registry_current#CLAIM 009]] · [[paper_registry_current#PAPER 061]] (`PMID 30755385`, knockout muscolare — è di là che arriva il ponte PDK1) · [[paper_registry_current#PAPER 062]] (`PMID 24871327`, asse lipidico).

#### Un negativo adiacente che fa lavoro positivo, e un programma con decorso opposto

`Supplement S4A` — mai citato dal testo corrente — mostra che sotto HFD le curve di peso di controllo e knockout sono **sovrapposte su tutti i 19 punti campionati**. L'eccesso tumorale sotto dieta lipidica (11/12 contro 4/18) **non** è quindi un confondente da obesità differenziale: è un nullo pulito che rafforza il risultato positivo che accompagna.

`Supplement S2B` porta **CTGF**, bersaglio canonico **YAP/Hippo**, elevato ~24× a 1 mese (`***`) e a 3 mesi (`*`), e **tornato al controllo** a 6 e 10 mesi. Il programma glicolitico si separa invece **solo** a 6 e 10 mesi. Due programmi trascrizionali con **decorso temporale opposto** nello stesso modello: il ramo Hippo si muove presto e si spegne, quello glicolitico si accende tardi. Questo è un argomento contro l'ordinamento causale proposto dagli autori — che tratta la deregolazione glicolitica come motore della proliferazione — perché la proliferazione (Ki67 `***` già a 1 mese, Fig 3e) e CTGF precedono di mesi il segnale glicolitico. `IPOTESI`, ancorata a `entries[37]` e `entries[24]`.

### DL-MECH-106 — **In SAOS-2 WWOX mRNA sits at ~45% of normal while WWOX protein is barely detectable: an mRNA-preserved / protein-lost signature the paper never connects to its own discussion of degradation**

- **Classification:** `DATO` on the two panels, both read from fingerprinted pixels · `INFERENZA` on the mRNA/protein discordance being real rather than assay noise · `IPOTESI` on a turnover route.
- **Source:** `PMID 20530675` — Kurek et al. 2010, *Cancer Research* 70(13):5577–86 (**not** *Oncogene*; see DL-REPO note below). Receipt `FTR-20260909-20530675-01` (`complete_fulltext_read`). Locator `entries[25]`. `file:files/figures/PMID20530675/nihms263536f2.jpg @ 29e83e08…`.
- **The finding.** Figure 2A labels every bar with its value: hBM 1.00, MG-63 0.10, KHOS 0.09, SAOS2 **0.45**, HOSC-1 0.08, U2OS 0.70, MCF-7 0.74, MDAMB231 0.15. Figure 2C is an eight-lane WWOX immunoblot in which **SAOS-2 carries a barely detectable band**, indistinguishable from KHOS — a line with **five times less** WWOX mRNA. The authors describe the four human OS lines collectively as *"barely detected"* at protein level and never remark that one of them retains nearly half of normal transcript.
- **Why it is a lead and not noise, and what it does NOT identify.** `mRNA preserved + protein lost` locates the loss **post-transcriptionally** and says nothing further. 🔴 **The cause remains unresolved and this paper excludes none of the alternatives:** accelerated **turnover**, **impaired translation**, **insolubility** (protein made but partitioning out of the extracted fraction), or a technical floor of epitope accessibility and antibody sensitivity — the blot is semi-quantitative with no loading-normalised densitometry. Collapsing it to degradation is the over-transfer already corrected once here: the P252A finding (normal mRNA, accelerated turnover, **no** rescue with MG-132, rescue with lysosomal probes, HSC70 co-IP) that `gold_is_in_the_details.md` carries as its worked example. The Discussion of this very paper says *"Alteration of WWOX in cancer has also been shown to be associated with LOH, HD and enhanced protein degradation"* — citing its reference 5 — and **never applies it to its own panel**, where the candidate is sitting.
- **An adjacent absence that sharpens it.** **U2OS — the only human OS line with high WWOX mRNA (0.70) — has no lane on the protein blot at all.** So the one line that could have discriminated "high mRNA → high protein" from "high mRNA → low protein" is the one line not measured. The same paper separately records that U2OS is one of the two lines where the WWOX/RUNX2 inverse relationship fails.
- **What it is NOT.** Not a demonstrated degradation route: there is no chase, no inhibitor, no half-life, no route determination anywhere in the paper. Not loading-normalised: panel C shows GAPDH but no densitometry, so "barely detectable" is semi-quantitative by inspection. Not a WWOX-DEE observation: this is a somatic osteosarcoma line and **nothing here transfers a phenotype** to the reference genotype. `ESPANSIONE` / T3.
- 🔴 **`PREMISE: DEFAULT_FROM_TEXTBOOK`** on the unwritten assumption that **low protein implies low transcription** — the assumption that let four lines be grouped as one phenotype. It is a research target, not a foundation, and it is already contradicted inside this single panel. The mirror guard also applies: the P252A → Q230P over-transfer was itself corrected on audit, so this lead must not be carried toward any WWOX-DEE allele.
- **`REVIVAL_TRIGGER`:** a cycloheximide chase in SAOS-2 with proteasomal versus lysosomal/CMA inhibitors, **together with** a soluble/insoluble fractionation and a polysome or metabolic-labelling readout — the three are what separate turnover from translation from insolubility, and any one alone would leave the other two standing. A loading-normalised WWOX protein quantification paired with its transcript is the cheap first step. Until then only the **location** of the defect is supported, never its route.
- **Interconnections:** [[framework/master/gold_is_in_the_details]] (the P252A worked example) · [[claim_registry_current#CLAIM 019]] (Q230P — named here only to mark the boundary this lead must **not** cross) · `PMID 16223882` (Fabbri 2005, the Ad-WWOX source, unread and assigned to `scientist-c` in a later wave of `AQEILAN-FT-C-001`).
- 🔴 **CORROBORATO IN UN SECONDO CONTESTO — 2026-09-09** (`scientist-c`, `PMID 21115974`, receipt `FTR-20260909-21115974-01`, locator `entries[11]`, `file:files/figures/PMID21115974/zh89991065650002.jpg @ bdd7b371…`). Lo stesso disaccoppiamento **mRNA preservato / proteina persa** compare in **sei linee T HTLV-I-positive e ATL**, cioè in un tessuto diverso e in una malattia diversa. Figura 2E (mRNA, % di Jurkat): C8166 ~75, MT-2 ~44, SLB-1 ~60, Hut102 ~38, TL-Om1 ~75, ED40515(−) ~9. Figura 2F (proteina, **percentili stampati sul pannello**): 22,4 · 28,9 · 21,1 · 11,8 · 51,3 · 3,95. **La proteina è più bassa dell'mRNA in tutte e sei.** Il paper lo dice in una sola riga — *«repression of WWOX expression was stronger at the protein level»* — e non ci torna più. ⚠️ **Limite portato con il dato, non staccato da esso:** i due pannelli usano **normalizzatori diversi** (β-actina per 2E, Hsp90 per 2F), sono corse singole, e **nessuno dei due porta marche di significatività**. È una **direzione consistente su sei linee**, non un disaccoppiamento testato. 🔴 **E il meccanismo proposto resta non mostrato anche qui:** gli autori attribuiscono la perdita proteica a ubiquitinazione e degradazione proteasomale *«through both Tax-dependent and -independent mechanisms (data not shown and see Figure 6D)»* — ma la Figura 6D **non ha alcuna corsia priva di Tax** (Tax copre tutte e nove) **e nessuna lettura di ubiquitina**. Entrambe le metà — quella *Tax-indipendente* e quella *ubiquitinazione* — poggiano sul non mostrato. **Che cosa cambia questa seconda occorrenza:** la firma non è una proprietà di SAOS-2 né dell'osteosarcoma. Resta però **localizzazione post-trascrizionale e nient'altro**: nessuno dei due contesti separa turnover, traduzione e insolubilità, e il `REVIVAL_TRIGGER` qui sopra vale immutato per entrambi. `ESPANSIONE` / T3 in entrambi i contesti — **nulla trasferisce al genotipo di riferimento**.

#### DL-METH-107 — a scope failure that modality screens cannot catch, plus the supplement rule it demonstrates

`PMID 20530675`'s Figure 5 is titled *"Inverse association of WWOX and RUNX2 expression in osteosarcoma"* while **Supplemental Table 8 shows no inverse association in any stratum of the 56 paired human cases**, and the biopsy stratum trends the **opposite** way (WWOX-strong tumours RUNX2-positive 7/9 = 78% versus 5/11 = 45% in WWOX-low; odds ratio **4.20 in the same direction**, Fisher p = 0.197; resections OR 1.10, metastases OR 1.20, pooled OR 1.46, p = 0.59). The authors hedge **correctly and in print** in the Results and again in the supplement legend. **The modality is intact; what travels beyond the evidence is the SCOPE of a title** — so a screen that looks for missing or weakened modals finds nothing here. `DL-METH-094` covers modality compression; **scope generalisation in a figure title is its sibling and needs its own check**, because the remedy differs.

The same reading demonstrates a second, cheaper rule. **Four of this paper's findings live only in the supplement and none is visible from the main text**: the reversal above; the penetrance contradiction (Figure S1's legend reports **19/22 = 86%** of knockout mice with tumours against the body's *"100%"* and Supplemental Table 1's 15+7=22, with the footnote *"only one femur was analyzed by microCT in 3 mice"* showing two animals were scored **bilateral on a single imaged femur**); the clone- and control-dependence of the rescue (Figure S4: parental HOS ~102 colonies, **empty vector ~60**, clone 11 ~8, **clone 8 ~48** — the vector alone removes ~41% and the two clones of one construct differ ~6-fold, while the main figure shows only clone 11); and the antibody gap in Figure S6, whose legend names a **rabbit polyclonal** anti-Runx2 that neither Methods section describes and for which the only Runx2 secondary described is **anti-mouse**. `IPOTESI` for the general rule, `DATO` for these four instances: **where a supplement is lawfully retrievable, the paired data that qualify a headline are disproportionately likely to be in it.**

### DL-THER-106 — **A responder subset to antiepileptic therapy exists in the published WOREE cohort, is visible only in the supplement, and the review's running text states the opposite as a flat negative**

- **Classification:** `DATO` on the tabulated per-patient response rows, read from the deposited supplement · `INFERENZA` on the responder subset being real rather than reporting noise · `IPOTESI`, and nothing stronger, on any regimen pattern.
- **Source:** `PMID 33916893` — Banne et al. 2021, *Cells* 10(4):824, DOI 10.3390/cells10040824. Receipt `FTR-20260909-33916893-01` (`complete_fulltext_read`). Locators `entries[12]` (the body's negative), `entries[20]`, `entries[21]`, `entries[22]` (the responder rows). `file:files/supplement/PMID33916893/TableS1_extracted.txt @ 4cefb752…`.
- **The finding.** Section 8 opens its therapeutic discussion with a flat negative — *"However, they are not effective, with most individuals becoming refractory to treatment"* — and cites Table S1 for it. Table S1's per-patient `AEDs-Responsed?` column **supports the word "most"** and simultaneously records responders the running text never mentions: **VPA + steroids — "yes"** (Emirati, homozygous exon-5 deletion); **PB + VGB + ACTH — "Yes"** (Romanian, `c.918del` E306fs, West syndrome); **PB + TPM — "Yes"** (Chinese, `c.229_230+2delGAGT`); one Italian patient **seizure-free since 2 y 3 m**; one Japanese patient **initially drug-resistant who then responded to lamotrigine plus sodium valproate**; and, in SCAR12, **two of the four P47T siblings on phenobarbital-containing regimens**. The review's own text carries none of this.
- **Why it is a lead and not noise.** These are six independent primary reports, each recording an explicit positive response in a disease the same paper describes as pharmacoresistant. A blanket "not effective" is the kind of negative this repository treats as the dangerous direction: *a false positive gets tested and dies; a false negative is silent, permanent and self-reinforcing.* If "WOREE does not respond to AEDs" propagates as background knowledge, nobody re-examines the responders, and the subset that did respond is exactly the population a mechanistic question would start from.
- 🔴 **What it is NOT, and the alternative explanation stated with the finding rather than after it.** **Hypsarrhythmia and West syndrome are frequent across this cohort, and ACTH and corticosteroids are standard of care for infantile spasms *generally*.** A responder subset concentrated on hormonal regimens is therefore at least as well explained by ordinary West-syndrome pharmacology as by anything WWOX-specific, and this table cannot separate the two. Further: `Yes` is **nowhere defined** in the supplement — it is not seizure freedom, a percentage reduction, or any stated endpoint; the rows come from **more than 21 heterogeneous primary reports** with no common protocol; there is **no control arm, no dosing, no duration and no follow-up standardisation**; responders may simply be milder phenotypes rather than drug responses; and publication bias runs toward reporting a success. **Not a treatment proposal, not a comparative-effectiveness signal, and not medical advice.**
- **It does not reopen a closed adjudication.** `therapeutic_overclaim_closure.md` row **A1** already fixes vigabatrin as *"symptomatic, does not modify WWOX; the most solid 'buy-time' base"*. This lead is consistent with that and adds nothing disease-modifying: symptomatic response is the only thing on the table here.
- 🔴 **`PREMISE: DEFAULT_FROM_TEXTBOOK`** on the unwritten assumption that **a syndrome described as "refractory" has no responders worth enumerating**. That assumption is what let a per-patient column with six positives be summarised as a flat negative by its own authors. It is a research target, not a foundation.
- **`REVIVAL_TRIGGER`:** any of — (1) the primary reports behind the six responder rows read at full text, with the actual endpoint behind each "Yes" recovered, which is the cheap first step and is purely acquisition work; (2) a WWOX-DEE cohort in which hormonal-therapy response is reported **stratified by the presence of infantile spasms**, which is the single comparison that would separate West-syndrome pharmacology from a WWOX-specific effect; (3) any prospective or registry series reporting seizure outcome by regimen in genetically confirmed WWOX-DEE. Until at least (1) and (2), only the **existence** of the subset is supported, never a regimen preference.
- **Interconnections:** [[claim_registry_current#CLAIM 001]] (vigabatrin/VABAM — `Status: conflicting evidence`; this lead touches the same drug class and changes nothing about that conflict) · [[paper_registry_current#PAPER 040]] · `PMID 29808465` (Johannsen — the Q230P sisters, recorded here as **refractory to multiple AEDs and to the ketogenic diet**, i.e. the reference allele sits on the non-responder side of this very table) · [[framework/master/gold_is_in_the_details]] (rule 5: the paired data that qualify a headline are disproportionately likely to be in the supplement — this is a second instance, after `DL-METH-107`).

### DL-MECH-107 — **WWOX has a regulatory architecture in intron 8, and after 68 deep dives this corpus has opened it exactly once**

- **Classification:** `DATO` on each of the four elements below as reported · `INFERENZA` that intron 8 is a regulatory region in its own right · `ESPANSIONE` on any transfer to the reference genotype.
- **Source:** `PMID 18674750` — Lee et al. 2008, *Am J Hum Genet* 83(2):180–192. Receipt `FTR-20260909-18674750-01` (`complete_fulltext_read`, 0 gaps). Locators: the tag-SNP design entry, the rs2738577/rs2550606 tagging entry, the intron-8 second-enhancer entry, and the Figure S2 panel attestation `file:files/figures/PMID18674750/S2_native.png @ 88f94f07…`.
- 🔴 **Measured, not asserted:** `grep -rlisE 'cis-regulatory|enhancer|intron 8'` over the 68 files in `deepdive_manifests/` returns **1**, and it is this paper's own manifest. Nine manifests mention HDL, lipid or cholesterol at all. **Every mechanism this corpus has read is protein-level.** Field density agrees: `WWOX AND cis-regulatory` returns **3** records on PubMed against **693** for `WWOX[tiab]` (2026-09-09).
- **What sits in that one intron — four independent things.** (1) `rs2548861`, inside a 211 bp computationally predicted cis-regulatory element within a 235 bp phastCons conserved element, with a demonstrated allele-specific effect on reporter activity in HepG2 (p = 0.0004 and 5.3 × 10⁻⁶ against the nonrisk haplotype) and allele-preferential nuclear-factor binding. (2) A **second** functional enhancer in the same intron, demonstrated *in vivo* in transgenic mice by a third party (ref 42 → `PMID 17086198`, `FT-071`). (3) `rs2667590`, the top regional hit of a lipid GWAS, in the same intron with **no LD** to `rs2548861` (r² < 0.02) (ref 12 → `PMID 18193043`, `FT-081`). (4) **Three distinct regions of homozygous deletion** in the same intron in AGS gastric adenocarcinoma lines (ref 43 → `PMID 10861292`, `FT-080`).
- **Why it is a lead.** A disease model built entirely on protein-level lesions has no object for "what regulates WWOX expression, and what happens when that regulation is perturbed rather than the protein". Two functional elements plus two independent association signals in one intron is enough to make that a subject rather than a curiosity.
- 🔴 **What it is NOT.** `rs2548861` is a **tag** capturing `rs2738577` (r² = 0.93) and `rs2550606` (r² = 0.96), neither separately genotyped, so the causal variant is unresolved and the functional work was done on this SNP **by choice of tag, not by genetic resolution**. The entire functional arm is **HepG2**, a hepatocyte line, and speaks to nothing neural. The reporter constructs *reduce* activity relative to empty vector, so the element is repressive **in this assay**, and whether the risk allele corresponds to more or less WWOX protein in vivo is **not established** — any claim that it lowers WWOX is `IPOTESI`. And the one arm that would test orientation-independence was run and never reported (see the dossier, §2.4).
- **`PREMISE: DEFAULT_FROM_TEXTBOOK`** on the unwritten assumption that **a tumour-suppressor gene's disease biology is a protein-level story**. That assumption is what makes a regulatory intron invisible for 68 readings. It is a research target, not a foundation.
- **`REVIVAL_TRIGGER`:** any of — (1) `PMID 17086198` read at full text, recovering what the transgenic enhancer assay actually showed and in which tissues; (2) any study reporting WWOX expression stratified by intron-8 genotype in **human brain or neural tissue**, which is the only thing that would make this axis touch the reference genotype; (3) any WWOX-DEE or SCAR12 cohort in which a non-coding intron-8 variant is reported.
- **Interconnections:** `FT-071` (updated 2026-09-09) · `FT-079`–`FT-082` · [[framework/master/gold_is_in_the_details]] rule 3 — a lipid-genetics paper carrying a mechanism class the entire syndrome literature has not supplied is the parity principle in its cleanest form: **the gene arrives here from outside its own literature entirely** (Pajukanta P: 2 WWOX papers out of 174).

### DL-METH-108 — **An Elsevier PDF text layer substitutes `¼` for `=` and the digit `3` for `×`, and only its control characters stand between it and a surface that would verify quotes nobody wrote**

- **Classification:** `DATO` on the measured substitution map · `DATO` on the guard behaviour, both branches executed.
- **Source:** `PMID 18674750`, `files/fulltext/PMID18674750_Lee2008.pdf @ b245190b…` against `files/fulltext/PMID18674750_Lee2008_PMC.html @ 3555e9e9…`. Receipt `FTR-20260909-18674750-01`.
- **The map, decoded quantitatively rather than guessed.** The PDF layer holds **74 `U+0002`** and **5 `U+0003`**; the clean HTML of the same article holds **exactly 74 `−`** and **exactly 5 `°`**. The one-to-one count match is what identifies the substitution instead of inferring it from context. On top of the controls sit **printable** substitutions: **`¼` for `=` (65 occurrences)**, **digit `3` for `×` (12 in `× 10` constructions)**, `b` for `β`, `0` for `′`. So the page's `p = 6.9 × 10⁻⁷` extracts as `p ¼ 6.9 3 10[U+0002]7`, and `MES = β × P (1 − P)` as `MES ¼ b 3 P (1 [U+0002] P)`.
- 🔴 **The measured gap, both branches run against `deepdive_manifest.py`'s own screen.** As extracted the surface is **REFUSED** — the C0 check fires. With the controls stripped, exactly as a well-meaning "repair" would, the surface is **ACCEPTED**, while 65 `¼`-for-`=` and 12 digit-3-for-`×` remain. The suspicion-by-absence check cannot save it either: the text contains 5 `<` and 9 `>` from `p < 0.05`, so `operators == 0` is false. **The only thing standing between this document and a surface that verifies quotes reading `p ¼ 6.9 3 10⁻⁷` is the presence of the control characters — which is precisely what the protocol warns a repair removes.**
- **Why the existing guard does not catch the printable half.** `PRINTABLE_SUBSTITUTIONS` carries three patterns — `q` for `±`, `D2` for `χ²`, `t` for `×`. Note the third: **the same glyph, `×`, is substituted as `t` by one extractor and as the digit `3` by this one.** A substitution table is a property of the *producer*, not of the corpus, so the pattern list grows one publisher at a time and its gaps are invisible until a document from a new producer arrives.
- **`REVIVAL_TRIGGER`:** any corpus PDF from a different producer whose statistical operators vanish while its control characters do not — that is the signature that a third substitution table exists and is unmodelled.
- **Interconnections:** [[framework/protocols/fulltext_read_receipt]] (*a text layer is not its page*) · [[framework/master/gold_is_in_the_details]] rules 5c–5e · the capability micro-upgrade this wave shipped, which adds the Elsevier signature to the printable screen.


### DL-METH-109 — **Il dato clinico c'è, il driver non è mai misurato, e l'unico trascritto virale misurato sta in una figura supplementare che il testo non cita mai**

- **Classification:** `DATO` sull'assenza di citazione e sull'assenza di qualunque misura di Tax nei pazienti (entrambe verificate meccanicamente) · `INFERENZA`, **condizionata a un'assunzione dichiarata**, sul mancato tracking WWOX/HBZ · reperto di **metodo**, non di biologia.
- **Source:** `PMID 21115974` — Fu J, Qu Z, … **Xiao G** (corresponding), *Blood* 2011;117(5):1652–1661. Receipt `FTR-20260909-21115974-01` (`complete_fulltext_read`, 35 locator). Locator `entries[16]`/`entries[17]`/`entries[18]`. `file:files/figures/PMID21115974/supplement_page3_200dpi.png @ d5ec90a9…`.
- **Il fatto.** Il paper conclude che la repressione di WWOX è *«an HTLV-I pathophysiologic event»* appoggiandosi alla **Figura 2G**, nove pazienti ATL primari. 🔴 **Tax non è mai misurato nel materiale dei pazienti** — verificato leggendo *Clinical samples* nei Metodi e ogni occorrenza di `ATL patients` / `primary ATL` / `PBMC` nel corpo. L'**unico** trascritto virale misurato in quegli stessi nove pazienti è **HBZ**, nella **Figura supplementare S3** — e il testo principale **non la cita mai**: `supplemental Figure 1` e `supplemental Figure 2` compaiono, `supplemental Figure 3` no, e la stringa **`HBZ` non compare da nessuna parte** nel corpo né nell'abstract.
- **I numeri, letti dai pixel** (S3 normalizzata ad ATL #1 = 100): controlli sani ~3 · ~0 · ~0; pazienti ATL #1 ~101, #2 ~13, #3 ~2, #4 ~3, #5 ~2,5, #6 ~23, #7 ~4, #8 ~1, #9 ~3,5. Contro la Fig 2G (WWOX, % del controllo #1): #1 ~35, #2 ~76, #3 ~41, #4 ~20,5, #5 ~31, #6 ~27, #7 ~26, #8 ~39,5, #9 ~100.
- ⚠️ **L'assunzione è dichiarata perché un audit cieco ha preteso che lo fosse.** *Nulla* nelle due didascalie, nei due pannelli o nei Metodi afferma che `ATL #1–#9` indichi **gli stessi pazienti nello stesso ordine** in 2G e in S3. **Su quell'assunzione** WWOX non segue HBZ: il paziente con WWOX più basso (#4, ~20,5) ha HBZ ~3, mentre quello con HBZ di gran lunga più alto (#1, ~101) ha WWOX solo moderatamente ridotto (~35). **Senza quell'assunzione** resta comunque in piedi, non condizionata, **l'assenza di qualunque misura di Tax nei pazienti**.
- **Perché è un reperto di metodo e non di biologia.** Nessun passaggio su titolo, abstract, didascalie o testo principale poteva trovarlo: **esiste solo nel supplemento, e solo nei pixel**. È la regola 4 di [[gold_is_in_the_details]] presa dal lato opposto rispetto a `DL-METH-079` — lì la superficie che viaggia *diceva il falso*, qui la superficie che viaggia **tace su un dato che il paper ha raccolto, depositato e non ha discusso**.
- **Che cosa NON è.** Non è un'accusa di occultamento: HBZ nel 2011 era appena entrato nel modello dell'ATL e la figura potrebbe essere il residuo di una revisione. Non è una confutazione del meccanismo Tax→NF-κB noncanonico→WWOX, che nelle **linee cellulari** è sostenuto da esperimenti mostrati. **È un limite sul solo anello clinico della catena** — l'unico che porta il paper dalle linee agli esseri umani.
- **`PREMISE: DATO`** sulla non-citazione (verificata meccanicamente sul corpo estratto) · 🔴 **`PREMISE: DEFAULT_FROM_TEXTBOOK`** sull'assunto implicito che *in ATL primario il driver sia Tax* — nell'ATL conclamato l'espressione di Tax è frequentemente silenziata mentre HBZ persiste, e il paper non affronta la questione. È un bersaglio di ricerca, non una fondazione.
- **`REVIVAL_TRIGGER`:** una misura **appaiata di Tax e HBZ** in materiale ATL primario insieme a WWOX riapre la questione in entrambe le direzioni — confermerebbe il modello se WWOX seguisse Tax, o lo sposterebbe su HBZ se non lo seguisse. In assenza di essa, la frase *«pathophysiologic event»* resta sostenuta da una correlazione senza il suo driver.
- **Interconnections:** `DL-METH-079` (una superficie che viaggia e dice il contrario del proprio paper) · `DL-MECH-106` (stessa fonte, firma mRNA/proteina) · [[claim_registry_current#CLAIM 023]] (corroborata sul titolo, **non** ristretta sul meccanismo da questa lettura) · [[gold_is_in_the_details]] regola 5 (*il supplemento è dove un titolo viene smentito*).


### DL-METH-110 — **Un archivio open-access può distribuire le figure a 532 px e insieme depositarne gli stream a 4× nel proprio PDF di rendering: la risoluzione «nativa» è una proprietà della rotta, non dell'articolo**

- **Classification:** `DATO` sulla misura delle due risoluzioni e sull'identità della rotta · reperto di **metodo**, disease-agnostico.
- **Source:** `PMID 41562193`, receipt `FTR-20260909-41562193-01`. `files/fulltext/PMID41562193_Druck2026_EuropePMC_render.pdf @ 9424664260a70125…` contro l'archivio `supplementaryFiles` della stessa richiesta Europe PMC.
- **Il fatto, misurato.** L'archivio `supplementaryFiles` di Europe PMC per `PMC12820907` consegna le tre figure principali come JPEG di **532×378**, **496×662** e **532×335**. Il **medesimo** contenuto è incorporato nel PDF `pdf=render` come stream immagine di **992×1323**, **2085×1313** e **2084×1479** — circa **4× lineari** per due delle tre. `Document.extract_image()` su xref restituisce lo **stream incorporato**, non una rasterizzazione della pagina: sono i byte figura del deposito, non una ricostruzione.
- 🔴 **Perché conta e non è un dettaglio di comodità.** A 532 px un pannello a 96 sostituzioni non è leggibile e una barra impilata non è segmentabile. Le tre affermazioni portanti di questa lettura — SBS40c assente da **ogni** campione Wwox (11 colonne di barre dentro la sola parentesi Fhit, **zero** dentro quella Wwox), il campione a carico mutazionale più alto che è un **wild-type**, e il 29,4% del segnale in firme che la legenda stessa dichiara artefattuali — **esistono tutte e tre solo nei pixel**, e nessuna sarebbe stata misurabile sulla risoluzione che l'archivio distribuisce. La regola 5c dice *figure ispezionate alla risoluzione originale*; questo caso mostra che **«originale» va cercata, perché la rotta più ovvia dello stesso archivio ne serve una degradata**.
- **La rotta, riproducibile.** `supplementaryFiles` → JPEG di display; `?pdf=render` → `extract_image` su ogni xref → stream nativi. Entrambe le versioni vanno fingerprintate e conservate: quella d'archivio è ciò che il repository *distribuisce*, quella xref è ciò su cui la lettura è stata fatta, e il dossier deve poter dire quale.
- **Il caso gemello, per il quale la rotta xref non esiste.** La Supplementary Figure 1 dello stesso articolo è **vettoriale e non contiene alcun raster**: lì i pixel vanno *prodotti*, non estratti, e la forma corretta è la ricetta della regola 5e — digest del PDF sorgente, pagina, dpi, renderer, digest dell'output (`page_renders` nel manifest). Le due situazioni si somigliano e vanno tenute distinte: **estrarre** uno stream esistente e **rendere** un vettore sono atti probatori diversi.
- **`PREMISE: DATO`** sulle misure di risoluzione, entrambe verificate sui file · **`PREMISE: INFERENZA`** sul fatto che lo stream incorporato sia la versione d'autore piuttosto che una versione intermedia del tipografo — non verificabile dall'esterno, e dichiarata perché la lettura vi si appoggia.
- **`REVIVAL_TRIGGER`:** un articolo in cui gli stream del PDF di rendering risultino **a risoluzione inferiore** ai file d'archivio, o visibilmente ricompressi rispetto ad essi, mostrerebbe che la gerarchia non è generale e che la rotta va scelta per articolo e non per repository.
- **Interconnections:** [[framework/master/gold_is_in_the_details]] regola 5c (*figure alla risoluzione originale*) e 5e (*pubblica la derivazione, non il derivato*) · [[framework/protocols/fulltext_read_receipt]] (*una didascalia non è la sua figura*) · `DL-METH-108` (superficie testuale degradata) — stesso difetto sull'asse delle immagini anziché del testo: **la superficie comoda non è la superficie probatoria**, in entrambi i casi.


### DL-MECH-111 — **Una minireview di Tier C definisce l'ubiquitinazione come rotta al proteasoma, cita ACK1, e così consegna a `DIS-001` la provenienza primaria che le mancava — restando essa stessa un'istanza della premessa falsa che l'aveva chiusa**

- **Classification:** `DATO` su ciò che la review afferma e su dove lo afferma · `INFERENZA` sulla struttura trasferibile (esistenza di una rotta di degradazione **regolata** per WWOX) · **fonte secondaria**: nessun claim guadagna o perde supporto probatorio da questa lettura.
- **Source:** `PMID 24510053` — Gardenswartz A, **Aqeilan RI**, *Exp Biol Med* 2014;239(3):253–263. Receipt `FTR-20260909-24510053-01` (`complete_fulltext_read`, `first_read`, 9 locator **tutti page adjudication**). Locator `entries[4]` (definizione proteasomale), `entries[5]` (ACK1), `entries[6]` (negativo E3). Crop `p02_ubiquitination_proteasome_definition.png @ b16b9ccbc01d…`, `p02_ack1_polyubiquitination.png @ ab623a24f57c…`, `p02_no_e3_ligase_known.png @ f8ea1d9dc520…`.
- **Il fatto.** La review definisce l'ubiquitinazione come la cascata che marca le proteine *«and thus designated for degradation via the proteasomal system»* — un **universale piatto**, senza **alcuna determinazione di linkage** in tutto il paper, citato al rif. 30, che è una review su **formazione ossea** priva di dati WWOX. È verbatim **`D-01`** della tabella `DEFAULTS THAT BIT US` (*polyubiquitination → proteasome*), il cui costo registrato è *«Discarded a testable ACK1/ITCH cascade for days»*.
- **E nella stessa sezione** nomina **Ack1**, tirosin-chinasi intracellulare che induce la poliubiquitinazione e la degradazione di WWOX in cellule di carcinoma prostatico, con il modale proprio degli autori — *«suggesting»* — sul passaggio causale, citando il **rif. 31 = PMID 16288044** (Mahajan, *Cancer Res* 2005): **la fonte primaria della cascata su cui poggia il rigetto attivo e riaperto `DIS-001`** («Inhibiting ACK1/ITCH will not rescue Q230P»), la cui premessa portante dichiarata è *«that this route ends in the proteasome»*.
- 🔴 **La convergenza, e il suo verso.** La review **fornisce** a `DIS-001` la provenienza che le mancava; **è** un'istanza fresca della premessa falsa che l'aveva chiusa; **non risolve nulla** sul linkage, perché non determina mai un tipo di catena. **Disposizione proposta: `DIS-001` rafforzata nella provenienza, verdetto invariato — resta riaperta.** La lettura che potrebbe muoverla è PMID 16288044, e `fulltext_receipts.py status` su quel PMID restituisce **una lista vuota**: non è mai stato aperto.
- **Un negativo datato, da non far viaggiare senza la sua data.** *«So far no ubiquitin E3 ligase was associated with WWOX under normal or disease states.»* È un'istantanea del **2014 già superata** — il thread ACK1/ITCH di questo stesso corpus nomina **ITCH**, una E3 ligasi, nel contesto WWOX P252A. `REVIVAL_TRIGGER` **soddisfatto all'arrivo**: ed è esattamente per questo che la data deve viaggiare attaccata alla frase. Un negativo di questa forma, portato avanti senza data, è il falso negativo silenzioso e auto-rinforzante che `epistemic_discipline` §2 esiste per impedire.
- **Densità di campo, misurata 2026-09-09.** `WWOX AND Ack1` restituisce **4 record** su tutto PubMed; `WWOX AND (E3 ligase)` 18; `WWOX AND ubiquitination` 24; `WWOX AND cancer` 524. La rotta di degradazione che questa review nomina è fra le letterature **più rade** che toccano il gene — ed è quella su cui pende un rigetto vivo.
- **Che cosa NON è.** Non è un'accusa alla review: nel 2014 la definizione proteasomale era il testo di scuola, ed è proprio per questo che `D-01` esiste. Non è una conferma che ACK1 causi la perdita di WWOX in malattia umana: il braccio umano è una **correlazione** fra p-Ack1 aumentato e WWOX ridotto in prostata, e gli autori scrivono *«suggesting»*. Non è trasferibile alla clinica del genotipo di riferimento: screening sul **corpo** (38.845 caratteri, lista dei 103 riferimenti separata prima) → **zero** occorrenze di `epilepsy`, `seizure`, `brain`, `CNS`, `WOREE`, `encephalopathy`, `ataxia`, `recessive`, `syndrome`, `Q230`, `SCAR12`.
- **`PREMISE: DEFAULT_FROM_TEXTBOOK`** su *polyUb → proteasoma*, qui asserito come **definizione**: non una fondazione, un bersaglio di ricerca. · **`PREMISE: INFERENZA`** sul fatto che il gruppo riporti accuratamente i propri fenotipi murini (rif. 13, 14), non verificato qui.
- **`REVIVAL_TRIGGER`:** una determinazione di linkage (K48 vs K63) su WWOX in qualunque sistema ritira l'ambiguità e rende decidibile `DIS-001`; la lettura di **PMID 16288044** è il passo che questa lettura raccomanda e deliberatamente non compie.
- **Interconnections:** [[dismissal_ledger_current]] `DIS-001` (provenienza) e `D-01` (istanza in natura) · `DL-METH-108` (superficie testuale degradata) — qui lo stesso difetto ha imposto **nove page adjudication**, prima applicazione della regola 5e in questo deployment · [[gold_is_in_the_details]] regola 1: un segnaposto **Tier C, «background corpus only», rilevanza clinica LOW, «no deep-dive performed»** conteneva la provenienza primaria di un rigetto attivo. Il tier ordinava la lettura; non ha mai autorizzato a saltarla.


### DL-METH-112 — **Una didascalia annuncia l'istologia dei tumori dell'esperimento e mostra due pannelli che sono **entrambi** bracci di controllo: il braccio sperimentale occupa zero pixel, e l'assenza è visibile solo aprendo l'immagine**

- **Classification:** `DATO` su ciò che il pannello contiene e non contiene, misurato sui pixel a 900 dpi · `INFERENZA` sulla struttura trasferibile del difetto.
- **Source:** `PMID 18460020` — Nakayama S, Semba S, Maeda N, Aqeilan RI, Huebner K, Yokozaki H, *Cancer Sci* 2008;99(7):1370–1376. Receipt `FTR-20260909-18460020-01` (`complete_fulltext_read`, `first_read`, 34 locator, 15/15 pannelli letti come pixel). Locator `entries[26]` (attestazione visiva, `panel_qualifies_text` → `entries[17]`), artefatto `fig2Bb_histology.png @ f42ebf51c18c14e1`.
- **Il fatto.** La didascalia della Figura 2 recita *«(b) Histological examination of the s.c. tumors (hematoxylin & eosin (HE). staining).»* Il pannello contiene **esattamente due** immagini H&E, etichettate a fuoco **`Control`** e **`Ad-GFP`**. **Non esiste alcun pannello Ad-WWOX.** Entrambe le immagini mostrate sono bracci di controllo: ciò che un lettore prende per un confronto istologico trattato-contro-non-trattato **confronta due tumori non trattati fra loro**.
- 🔴 **Perché non è un'accusa ma una struttura.** L'assenza è la conseguenza **onesta** del pannello (c): nel braccio Ad-WWOX non si è formato **alcun** tumore misurabile in 7 settimane su 5 topi, quindi non c'era nulla da sezionare. Il difetto non è il dato — è che **né la didascalia né il testo corrente lo dicono**, e che la sola via per accorgersene è aprire l'immagine e contare i pannelli.
- **La forma trasferibile, ed è questa la capitale.** *Quando un braccio sperimentale produce **zero** eventi, il pannello che dovrebbe mostrarlo non contiene un risultato negativo: non contiene nulla.* Un negativo assente e un negativo non mostrato sono indistinguibili in didascalia e distinguibili solo nei pixel. **Il controllo da eseguire è aritmetico e costa secondi: contare i pannelli dichiarati dalla didascalia e i pannelli presenti nell'immagine, e chiedere quale braccio manca.**
- **Terza occorrenza in quattro ondate per questo attore**, e le tre non si somigliano per caso: una figura ripubblicata che aveva perso il proprio controllo su osso normale; un titolo di figura smentito dalla propria tabella supplementare; e qui un braccio sperimentale a zero pixel dentro la parentesi cui è attribuito. **Il denominatore di una figura non è dichiarato da nessuno schema** (cfr. l'item aperto n. 1 di [[fulltext_read_receipt]], `panel_coverage` senza base del denominatore): finché non lo è, il conteggio dei pannelli resta un atto umano e va scritto nel dossier.
- **`PREMISE: DATO`** sul contenuto del pannello, letto a 900 dpi dal PDF fingerprintato · **`PREMISE: INFERENZA`** sul fatto che l'assenza discenda dal risultato nullo del pannello (c) e non da una scelta editoriale — coerente con i dati ma non dichiarata dagli autori.
- **`REVIVAL_TRIGGER`:** una didascalia che dichiari esplicitamente *«il braccio X non ha prodotto tumori sezionabili»* mostrerebbe che il difetto è evitabile a costo zero e va richiesto, non solo rilevato.
- **Interconnections:** [[fulltext_read_receipt]] (*una didascalia non è la sua figura*; item aperto 1, denominatore dei pannelli) · [[gold_is_in_the_details]] regola 5c (*figure ispezionate alla risoluzione originale, mai lette attraverso una conversione testuale*) · `DL-METH-108` e `DL-METH-110` (la superficie comoda non è la superficie probatoria) · [[epistemic_discipline]] §2 (*l'asimmetria dei negativi*): qui il negativo non è stato scartato, è stato **reso invisibile dalla propria riuscita**.


### DL-MECH-113 — **WWOX → Smad4 a livello proteico regge una densitometria ostile; il meccanismo «post-trascrizionale» che gli è attaccato poggia invece interamente su un «data not shown»**

- **Classification:** `DATO` sull'aumento di Smad4 proteico, corroborato da misura indipendente · `IPOTESI` sul meccanismo post-trascrizionale · `ESPANSIONE` / T3 verso il genotipo di riferimento.
- **Source:** `PMID 18460020`, come sopra. Locator `entries[15]` (mRNA non mostrato), `entries[19]` (conclusione del Discussion), `entries[33]` (densitometria sullo stream incorporato autentico `xref23_83x146.jpeg @ 92e85fa0676f2087`).
- **La misura, fatta per attaccare la claim e che la conferma.** Densitometria per corsia sullo **stream JPEG 83×146 depositato** — non su un re-render — con fondo al 90° percentile per striscia: β-actina No-transfection 1,00 / Mock 1,35 / pWWOX 1,37; Smad4 1,00 / 0,96 / **2,63**. Mock e pWWOX sono **ben appaiate per carico**, e Smad4 pWWOX-su-Mock vale **2,73× grezzo e 2,69× dopo normalizzazione**. 🔴 **L'ipotesi che il controllo di carico gonfiasse il risultato era mia, è stata misurata, ed è falsa.** Registrata con lo stesso rilievo di ciò che ha retto.
- **Ciò che la stessa misura mostra e il paper non dice.** **Smad2/3 — il controllo di specificità implicito — non è invariante**: 1,42× in Mock e 1,23× in pWWOX dopo la medesima normalizzazione. Gli autori non riportano **alcuna** densitometria e **alcun** numero di repliche per questo blot.
- 🔴 **Dove la catena si spezza.** *«No alteration was found at the messenger RNA (mRNA) levels of Smad4 expression (data not shown).»* È l'**unica** base del passaggio a *«Wwox-mediated post-transcriptional regulation of Smad4 levels»*, ripetuto nell'abstract e nel Discussion. Il meccanismo principale del paper poggia **interamente su un negativo mai mostrato**.
- **`PREMISE: DEFAULT_FROM_TEXTBOOK`** — che mRNA invariato con proteina aumentata implichi controllo **post-trascrizionale**. Non lo implica: non esclude una regolazione dell'**inizio della traduzione**, che è regolazione a livello di trascritto di altro tipo. Un default di questa forma è un **bersaglio di ricerca, non una fondazione**.
- **La rotta candidata che il paper stesso indica e non prova.** Smad4 è degradato via poliubiquitinazione da **Smurf1/Smurf2**, E3 ligasi **a dominio WW**; gli autori notano che i Smurf non legano Smad4 direttamente ma via motivi **PPXY** in Smad2/Smad7. WWOX è una proteina a dominio WW che lega PPXY. **Ipotesi competitiva mai testata qui: WWOX stabilizza Smad4 competendo con Smurf per gli stessi motivi PPXY.** `IPOTESI`, non misurata in questo paper in alcuna forma.
- **Densità di campo, misurata 2026-09-09.** `WWOX AND SMAD4` **18** record; `WWOX AND Smad*` **28**; `WWOX AND pancrea*` **14**; 🔴 **`WWOX AND PanIN` = 1, ed è questo paper**: l'idea che la perdita di WWOX sia un evento **precoce** che segue il grado della lesione precursore non è stata riprodotta indipendentemente in diciotto anni. Non è una prova contraria; è la ragione per cui non ci si può appoggiare sopra.
- **`REVIVAL_TRIGGER`:** (a) qualunque misura pubblicata di mRNA Smad4 sotto restituzione di WWOX decide il passaggio post-trascrizionale; (b) un esperimento di competizione WWOX/Smurf su motivi PPXY promuoverebbe l'ipotesi sopra da `IPOTESI` a rotta testabile; (c) un secondo record indipendente su WWOX e PanIN toglierebbe l'isolamento al nodo precursore.
- **Interconnections:** [[claim_registry_current#CLAIM 007]] e [[claim_registry_current#CLAIM 024]] (funzione WW/PPXY — **nessun link creato**, il sistema qui è un carcinoma pancreatico) · `CC-20260909-18460020-01` §3 (il **non-link** deliberato verso `CLAIM 002`/`004`) · `DL-MECH-111` (poliUb → proteasoma come default che ha già morso questo corpus: **lo stesso default compare qui sull'asse Smurf/Smad4**) · [[gold_is_in_the_details]] regola 3 (*il contesto di malattia non è mai una ragione per declassare*).
