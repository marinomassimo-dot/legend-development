...
- 🔴 **AGGIORNAMENTO — 2026-09-21: il «rischio teorico forte» di questa voce non è più teorico, ed è ora sostenuto da un laboratorio ESTERNO, in NEURONI.** `PMID 35984507` — Carvalho C, Correia SC, Seiça R, Moreira PI 2022, *Cell Mol Life Sci* 79(9):487, [DOI](https://doi.org/10.1007/s00018-022-04508-7), **Center for Neuroscience and Cell Biology / CIBB, Università di Coimbra**, nessun legame con NCKU. **Il titolo è il reperto:** *"**WWOX inhibition** by Zfra1-31 **restores** mitochondrial homeostasis and viability of neuronal cells exposed to high glucose."* Zfra1-31 vi è descritto come *"**the specific inhibitor of WWOX**"*, e la direzione è esplicita: *"the **activation of WWOX preceded** mitochondrial dysfunction and cell death"*; *"the **inhibition of WWOX with Zfra1-31 reversed**, totally or partially, the alterations promoted by high glucose."*
  - **Che cosa cambia:** l'enunciato falsificabile di questa voce ipotizzava che Zfra potesse agire *via una rete WWOX-independent o context-specific*. **Un gruppo indipendente lo usa, e lo descrive, come inibitore di WWOX.** Il beneficio che riporta **dipende dal fatto che ci sia WWOX da inibire.** In un genotipo carente — e a maggior ragione in un composto eterozigote con un allele missenso che produce ancora proteina — **questo è esattamente il verso sbagliato**, e non è correggibile con dose, via o formulazione: è un'obiezione di categoria.
  - **Status invariato a `flagged` — «monitorare ma non promuovere» era la posizione giusta e lo resta**, ma la ragione si rafforza: non è più «rischio teorico», è «un laboratorio esterno lo impiega per ottenere l'effetto opposto a quello che a noi serve».
  - ⚠️ ~~**Onestà su che cos'è questo aggiornamento:** poggia su un **abstract**, non su una lettura.~~ **SUPERATO — 2026-09-21: il paper è stato LETTO.**
  - 🔴 **2026-09-21 — LETTURA COMPLETA di `PMID 35984507`, main + supplementary** ([`A9_fulltext_read_20260921.md`](../analysis/A9_fulltext_read_20260921.md); receipt `FTR-20260921-35984507-01` e `-02`; PDF forniti dall'operatore). Le due domande poste sopra hanno risposta, e **la seconda cambia la formulazione di questa voce**:
    - **Braccio genetico su WWOX: NON ESISTE.** `siRNA`, `shRNA`, `knockdown`, `knockout`, `CRISPR`, `silenc*`, `transfect*`, `lentivir*` = **0**, nel main *e* nel supplementary. Nessun peptide **scrambled/S8G** come controllo di specificità (`scrambled` 0, `S8G` 0); l'unica affermazione di specificità è *«does not affect mitochondria and cells function under control conditions **(data not shown)**»*.
    - **WWOX totale: MAI MISURATO, benché l'anticorpo fosse in casa.** La tabella anticorpi elenca *sia* `Anti-WWOX (phospho Y33) Abcam (ab193624)` *sia* `WWOX Merck Millipore (ABN413)`, e la normalizzazione dichiarata è *«β-actin was used as a loading control»*. Nessun dato di WWOX totale, nessun rapporto pWWOX/WWOX, in nessuna legenda o frase.
  - 🔴 **Conseguenza per l'enunciato falsificabile di questa voce — e non è una sfumatura.** Un segnale pTyr33 normalizzato su β-actina scende se scende la fosforilazione **oppure** se scende la proteina WWOX. Poiché la zfration prende WWOX **come substrato**, il ~64% di riduzione riportato è **esattamente** la quantità che non distingue *«Zfra ha inibito WWOX»* da *«Zfra ha consumato WWOX»*. Quindi A9 **non è una verifica indipendente** del verbo «inibizione»: è compatibile con entrambe le letture.
  - **Ma il verso non cambia, e la conclusione terapeutica si rafforza:** inibizione e consumo puntano **nella stessa direzione** in un genotipo il cui problema è *troppo poca WWOX funzionale*. Resta **obiezione di categoria, non di dose**. **Status invariato a `flagged`.**
  - ⚠️ **Formulazione corretta, da usare ovunque questa voce venga citata:** *«Evidenza neuronale indipendente che Zfra1-31 è protettivo riducendo il segnale pY33-WWOX; l'attribuzione causale della protezione all'**inibizione** di WWOX resta irrisolta — nessuna perturbazione genetica ortogonale di WWOX, nessun controllo con peptide inattivo, e WWOX totale mai misurata.»*
  - ⚠️ **`A9` è CHIUSO.** Nessuna ulteriore acquisizione serve per questo paper. **REVIVAL_TRIGGER:** una pubblicazione che riporti **WWOX totale** accanto a pTyr33-WWOX sotto Zfra1-31, o un braccio genetico su WWOX accanto al peptide in modello neuronale, riapre la questione — quella singola misura decide fra le due letture.
...
- 🔴 **RICALIBRAZIONE 2026-09-21 — il meccanismo specifico è più debole di quanto questa voce assumeva, e la review del campo dice l'opposto sul cervello.** Letti per la prima volta i due paper del metabolismo (`FTR-20260921-35328751-01`, `FTR-20260921-36271927-01`; audit [`wwox_metabolism_reviews_audit_20260921.md`](../analysis/wwox_metabolism_reviews_audit_20260921.md)). Il dettaglio sta in [[discovery_ledger_current#DL-MECH-020 — WWOX-loss induce un fenotipo Warburg-like via HIF1α: razionale meccanicistico WWOX-specifico per la dieta chetogenica|DL-MECH-020]]; quel che tocca **questa** ipotesi:
  - 🔴 **Il blocco PDK↑ ⊣ PDH — il collo di bottiglia che la KD bypasserebbe, ed è il cuore del razionale «WWOX-specifico» — nelle cellule umane poggia sul SOLO TRASCRITTO DI PDK.** L'attività PDH e la citrato-sintasi sono state **misurate e sono invariate in tutte e quattro le condizioni**. **Il razionale non è confutato, ma il passo che lo rende WWOX-specifico non è misurato in nessun sistema umano.**
  - 🔴 **La review del campo, sul cervello, va nella direzione opposta:** *"It has been shown that **increasing glycolysis may slow neurodegeneration**."* Questa voce propone di spostare il metabolismo **via dalla glicolisi**. La tensione non è risolta da nessuno e va portata al team curante insieme al razionale, non taciuta.
  - **Le due fonti non sono indipendenti fra loro** (un primario e la review dello stesso gruppo su quel primario) e **nessuna delle due propone un intervento metabolico**: `ketogenic`, `dichloroacetate`, `metformin` = zero in entrambe.
  - ✅ **L'esperimento minimo non cambia e sale di priorità:** il **Seahorse OCR/ECAR su cellule donor-derived** resta l'unico falsificatore — e ora si sa che **non è mai stato fatto su WWOX in alcun sistema umano**, il che lo rende anche il primo, non solo il decisivo.
  - **Che cosa NON cambia:** la KD resta standard-of-care nelle encefalopatie epilettiche e il suo uso empirico non dipende da questo razionale. ⚠️ **E il dato clinico più pertinente di oggi viene da altrove:** in `PMID 35573960` la dieta chetogenica *"did not significantly improve seizures and was suspended"* in un paziente WOREE biallelico-null — `n = 1`, genotipo null/null, esordio primo-giorno, **non una confutazione per un genotipo con proteina residua**, ma va contato.
  - **Nulla di tutto questo è parere medico.**
- **Ranking (aggiornato 2026-09-21):** azionabilità 3/3 · **evidenza 2/3 → 1/3** (il passo WWOX-specifico non è misurato nell'uomo, e la review del campo lo attraversa in direzione opposta sul cervello) · sicurezza 3/3 · time-to-benefit 3/3 · valore-parziale 2/3 → **12/15**
- **Ranking (precedente, 2026-07-10c):** azionabilità 3/3 · **evidenza 2/3** (il Warburg è documentato in modelli, mai nel paziente) · sicurezza 3/3 · time-to-benefit 3/3 · valore-parziale 2/3 (KD abbandonata in 2/3 dei casi WWOX in cui è stata tentata) → **13/15**
...
- 🟢 **AGGIORNAMENTO — 2026-09-21: il ramo degradativo di questa ipotesi ha ora un precedente DIMOSTRATO su una sostituzione missenso di WWOX, e il precedente porta con sé anche l'incertezza n. 1 intatta.** Fonte: `PMID 41124647` (Zhang X *et al.*, senior Hong B / Wang H, *Adv Sci* 2025;13(1):e07602, **Chinese Academy of Sciences, Hefei — indipendente da Chang/NCKU**), letto integralmente; ricevuta `FTR-20260921-41124647-01`; audit [`wwox_missense_cma_degradation_audit_20260921.md`](../analysis/wwox_missense_cma_degradation_audit_20260921.md). Citazioni verificate personalmente.
  - ✅ **Il ramo «degradazione prematura» non è più solo un'alternativa logica: per `p.P252A` è misurato.** mRNA non ridotto, proteina ridotta ⇒ perdita **post-trascrizionale**; e la via è **lisosomiale, non proteasomiale, non macroautofagica** — *"the proteasome inhibitor, MG-132 treatment, **did not induce** the accumulation"*, 3-MA negativo, cloroquina e NH₄Cl positivi.
  - 🟢 **Esiste un braccio di blocco, e va aggiunto all'esperimento minimo di questa ipotesi:** cloroquina **40 µM / 24 h** e NH₄Cl **250 µM / 24 h** ripristinano il livello proteico del mutante. **Questo rende l'esperimento di discriminazione più economico**: al pulse-chase e alla misura di solubilità si affianca un braccio inibitore lisosomiale, che se positivo colloca il collo di bottiglia nel **turnover** e non nella **sintesi** — che è esattamente la biforcazione che questa ipotesi chiede di risolvere.
  - 🔴 **E l'incertezza n. 1 di questa voce — «stabile-ma-inerte» — resta in piedi, anzi è illustrata dal paper stesso.** Sotto rescue il paper misura **solo l'intensità di banda al Western**: **nessun saggio funzionale** viene eseguito — né clonogenicità, né invasione, né comet, né γH2AX, né NHEJ. **La via è bloccabile; se bloccarla restituisca funzione è non testato.** Il precedente conferma che il ramo esiste; **non** conferma che il rescue sia funzionale.
  - ⚠️ **Distanza da dichiarare, e non va coperta.** (a) Il sistema è **CAL-62 (carcinoma tiroideo) che sovraesprime stabilmente un transgene Flag** — non fibroblasti di paziente, non neuroni, non proteina endogena. (b) `SDR`, `ADH` e `short-chain` compaiono **zero volte** nel corpo: **la collocazione dei residui nel dominio SDR è aritmetica del lettore, non affermazione del paper.** (c) **Solo `P252A` è l'allele degradato**; `P282A` perde funzione **senza difetto di stabilità**, il che è un promemoria che una sostituzione nello stesso dominio può rompere la funzione senza toccare l'abbondanza. (d) Per `P252A` **abbondanza e funzione sono confuse e mai separate** — lo stesso paper spiega la perdita di un co-IP con *"likely due to the **low abundance**"*.
  - 🔴 **Non importare «autofagia mediata da chaperone» come stabilita:** gli autori scrivono *"**we speculated** that lysosomal degradation … occurred through chaperone-mediated autophagy"* mentre l'abstract la dà come dimostrata; il motivo KFERQ-like **non è mai stato mutato** e `LAMP2A`/`LAMP2` compaiono **zero volte**. Registrare come **degradazione lisosomio-dipendente, proteasoma-indipendente, macroautofagia-indipendente, con co-IP HSC70 e motivo predetto.**
  - 🔴 **Freno umano che questa ipotesi deve portare:** il probando di quel paper, **omozigote per due missenso dello span SDR**, i cui mutanti sono **indistinguibili dal vettore vuoto** in ogni saggio oncologico, *"**did not suffer from WWOX-related nervous system disease**"* a 35 anni, e gli autori stessi propongono *"may have some **residual protein function** to maintain the development of the neurological system."* **«Morto ai saggi oncologici» ≠ «nullo neurologicamente»:** nessun saggio tumorale può essere usato per predire la gravità neurologica attesa di un allele missenso.
  - **Ranking e status invariati.** Il precedente rende il programma **più eseguibile**, non più promosso: non promuovere alcuna molecola, e in particolare **non proporre la cloroquina come terapia** — qui è un reagente di discriminazione meccanicistica in coltura, nient'altro. **Nulla in questa voce è parere medico.**
