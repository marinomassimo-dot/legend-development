...
| **`verify_the_omitted_clause`** — when checking a delegate's or a paper's headline, read what sits *beside* the quoted sentence, because the transmitted clause is chosen for the claim and the adjacent one is chosen for nothing | 🔴 **Three instances in a single wave, each the highest-value item of its file.** (a) Atanasov 2007's *"rather than … loss of catalytic activity"* — the clause that decides whether the precedent transfers at all, omitted from the report. (b) Hamdan 2006's `BRET_50`-vs-`BRET_max` paragraph — a **second free discriminator** two paragraphs past the quoted one. (c) The TX-007 Methods' **empty parenthesis** beside a populated one — extraction damage visible only in the neighbouring token | **Not yet observed to fail**, and its cost is the highest on this page: it requires fetching the source rather than trusting the quote. On a correct report it returns nothing | **3** | 🟢 **strongest candidate on this page — and the newest, so treat the count with suspicion** |
...
⚠️ **And it is the one whose count I trust least**, precisely because three instances arrived
together in a single wave, under a single orchestrator, on a single day. Three correlated
observations are not three independent ones. It needs a different wave before the count means
what it looks like.

**Still not said.** Only `enumerate_baseline_before_scoring` has ever been **observed to fail**,
and its only instance *is* the failure. Six primitives with no recorded failure remain *untested
