...
| **`enumerate_baseline_before_scoring`** — before judging anything "new", enumerate *every* repository file bearing the identifier, by listing, not by guessing filenames | Emerged **from its own absence**: §3 of the re-read missed `PMID34747138_partial_locators.md`, a second dossier for the same PMID. Two rediscoveries would have been graded as discoveries. 🟢 **(d) FIRST SUCCESS, and the first time it fired *before* the writing rather than after:** I was about to report that Wang 2011's purified GST-WWOX / GSK3β / Tau kinase-inhibition assay satisfies the session's binding chaperone condition (d) — a genuinely large claim. Grepping the per-molecule census **first** showed it already scored exactly that assay: passing criteria 1–3, conditional on 4. The claim survived in a **narrower and more useful form** (a readout exists and has never been run on a disease allele) instead of being published as a discovery and corrected later. **(b) Same day, different actors:** two delegates independently proposed as a *first* measurement an experiment whose published prior (Johannsen 2018, `PMID 29808465`) sits in the discovery ledger under a heading naming the allele — see `q230p_three_designs_one_experiment_20260922.md` | Its only instance is a **failure**, not a benefit. It has never yet been run *as* a primitive | **4 — three failures and 🟢 ONE SUCCESS** | 🟡 **promoted from 🟠. It finally worked, and it worked preventively** |
...
| **`outcome_distribution_width`** — an experiment's value is the width of its outcome distribution, not the importance of the quantity it measures | Caught my own endorsement of a host-reference qPCR whose outcome was **arithmetically bounded** (mtDNA ≈0.06 pp, ploidy ≈0.03 % against the 8.75–12.05× needed): *a foreseeable result is a validation, not a discriminator.* Changed the recommended next experiment. 🔴 **(b) WITHDRAWN — and the withdrawal is worth more than the instance was.** I widened Scientist K's blot with a 26–30 °C arm; Scientist B refuted it and I verified the refutation (Roobol 2008, `PMID 19054067`: mild hypothermia is itself a global degradation inhibitor, so the arm cannot separate *folded* from *no longer cleared*). **The primitive fired correctly and I applied it to the wrong instrument** — I widened the outcome distribution of a readout that could not carry the discrimination | 🔴 **OBSERVED TO FAIL, once, and by misapplication rather than by the rule being wrong.** Widening an outcome distribution is worthless if the readout being widened cannot carry the discrimination. The primitive as stated does not say *check that the added arm perturbs only the variable in question* — and that omission is what let me add a temperature arm that moves synthesis, degradation and the chaperone complement at once | **1**, plus **1 failure** | 🟡 **promising, under-evidenced — and now with its first recorded misapplication** |
...
**Updated the same day: a second primitive has now been observed to fail.**
`outcome_distribution_width` misfired when I applied it to a readout that could not carry the
discrimination (the withdrawn 26–30 °C arm). That is the first failure on this page from a
primitive that had a *successful* instance, and it is the more informative kind: the rule was
sound, the application was not, and the rule as written contains nothing that would have stopped
me. **A primitive that cannot be misapplied has not been used enough.**

**Still not said.** `enumerate_baseline_before_scoring` has been **observed to fail**,
and **all three** of its instances *are* failures — the second (two delegates proposing as novel an
experiment published in 2018) came from a different actor than the first, which is why it counts
as evidence and not as a repeat. Five primitives with no recorded failure remain *untested
