...
> ### 🔴 DE-IDENTIFICATION NOTE — what the tables below deliberately do not say
>
> **Both variant tables were edited after first drafting to remove a reidentification vector, and the
> vector is worth naming because it is easy to reintroduce.** An allele-level census naturally wants
> to write *"this variant, in trans with that variant, in a family of such-and-such origin"* — and
> that triple is precisely what turns a disease-model worked example back into **one identifiable
> family**. The public edition's rule is that *"specific variants appear only as decoupled
> disease-model worked examples … never as one persistent individual's inherited alleles."*
>
> So the tables give, per allele: **its own identity, its class, its reported patient and family
> counts, and its zygosity in aggregate.** They do **not** give the specific trans partner of a
> compound-heterozygous case, and they do **not** attach a reporting population to a variant pair.
>
> ⚠️ **One of the two instances was caught by `public_release_gate.py`
> (`REIDENTIFYING_VARIANT_COMBINATION`) and the other was not** — the gate flagged the splice-table
> row and passed the missense-table row, which stated the same pairing more explicitly and added
> population descriptors. **The gate is a floor, not a ceiling.** Both were removed by hand.
>
> **What is deliberately KEPT:** the country or cohort of a *published study* (e.g. "a 7-patient
> Argentine series", "exome sequencing in a high-consanguinity population"). That is ordinary
> bibliographic description of a citable paper and it is what makes the ascertainment-bias argument
> in § 5 checkable. The distinction held throughout: **a cohort has a provenance; an individual's
> allele pair does not get one.**
...
| `c.517-2A>G` | canonical **acceptor**, intron 5 → exon 6 | ✅ | ⚠️ **MEASURED (reported)** — *"Complementary DNA sequencing demonstrated…"* exon 6 skipping; LEGEND has **never seen the measurement** | ≥2, plus 2 independent WOREE iPSC lines | reported in a compound-heterozygous setting; ⚠️ *the specific trans partner and the reporting population are deliberately not reproduced here — see the de-identification note below the table* | 30853297 · 33916893 · 42397075 | **GENE-REPLACEMENT** (skipping → null-like; `CLAIM 018`) | MODERATE for pathogenicity, LOW for rescuability | 🔴 **ABS** for the measurement; **DOS** for the patient count |
...
| `c.229_230+2delGAGT` | donor-spanning indel | ✅ | ❌ | 1 (compound het) | compound het | 33916893 Table S1 | **GENE-REPLACEMENT** | LOW | **DOS** |
...
|---|---|---|---|---|---|---|---|---|---|
| `c.689A>C` **p.(Gln230Pro)** | missense, buried SDR core, α-helix 227–232, relSASA 0.00 | ✅ ThermoMPNN +1.51 kcal/mol; ESM-2 −9.08 LLR (worst substitution at the site) | ⚠️ **protein-level endpoint only** — patient fibroblasts: *qRT-PCR normal WWOX transcript; Western blot: protein not detected*; authors leave **"impaired translation **or** premature degradation"** undiscriminated | **8 patients / 6 families** (**4 homozygous, 4 compound heterozygous**). Independently: **4 families** in the 2019 series. **2 individuals** in the 2025 registry cohort. ⚠️ *Reporting populations and the specific trans partners are deliberately not reproduced — see the de-identification note below.* | both | 33916893 (census) · 30356099 (4 families) · 29808465 (function) · 40875931 | **PROTEIN-RESCUE (conditional)** — 🔴 and the conditionality is not rhetorical: no synthesis, solubility, half-life, route or function measurement exists for this allele | MODERATE for pathogenicity (`CLAIM 019`, consolidated baseline); **LOW** for recoverability | **DOS** + **FT**; 🔴 **ABS** for the Johannsen functional endpoint (LEGEND holds only the abstract) |
