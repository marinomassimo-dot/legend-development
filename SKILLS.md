...
The reasoning power of LEGEND does not live in one big prompt. It lives in **23 composable skills** and **5 reusable subagents** that call each other in a defined order, each with its own gate, its own output contract and — where it exists — its own runnable code.
...
| [`legend-start`](.claude/skills/legend-start/SKILL.md) | Session boot: selects task-specific context, runs structural LINT, declares `READY` or `BLOCK`. | SPECIFIED |
...
| [`legend-capability-scout`](.claude/skills/legend-capability-scout/SKILL.md) | The evolutionary radar. At every close it records a reusable capability gap if one emerged — `CHANGE = NONE` is a valid outcome — and it scouts the ecosystem when a named gap, the weekly schedule or the operator asks. This is the engine behind the compounding claim. | SPECIFIED |
...
| Skill | What it does | Status |
|---|---|---|
| [`legend-hypothesis-forge`](.claude/skills/legend-hypothesis-forge/SKILL.md) | The co-scientist loop: generate → critique → rank → evolve, producing scored candidate therapeutic hypotheses — as many mechanistically distinct ones as the explanatory space holds, each with a discriminating test — with a mandatory epistemic tag and a safety gate. Ships a knowledge-graph slice builder, [`kg_thin_slice.py`](.claude/skills/legend-hypothesis-forge/scripts/kg_thin_slice.py) (queries Monarch/DGIdb, so it needs network), and a [ranking rubric](.claude/skills/legend-hypothesis-forge/references/ranking_rubric.md). | IMPLEMENTED · online |
...
| [`legend-discovery-method`](.claude/skills/legend-discovery-method/SKILL.md) | An **optional** toolkit of seven reasoning primitives (enumerate the baseline, diverge, connect domains, preregister, compress an experiment, recursive reread, adversarial re-retrieval) for questions that need more than a direct lookup. Gates nothing; using one primitive alone is the normal case. | SPECIFIED |
