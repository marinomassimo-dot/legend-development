...
- **Capture verbatim locators while the document is open.** For every statement the reading will carry out, record the exact sentence that supports it: `proposition` (what it is evidence for), `snippet` (verbatim, ≥30 characters), `anchor` (section, figure or table). They go in the work manifest under `verbatim_locators` and are validated by `framework/scripts/deepdive_manifest.py`. A receipt attests that the document was read; it does not attest which sentence supports which statement. Recovering a quote afterwards costs the reading a second time. If the reading supports no proposition, waive the section with an argument ≥40 characters — waiving is legitimate, silence is not.
