...
- **Assembly** (do this periodically on the ledger, INDRA-style): (a) *dedup* — merge equivalent statements from different papers; (b) *contradiction resolution* — if two papers give opposite relations, mark `conflicting` and keep both sources; (c) *missing-link inference* — if A→B and B→C, propose A→C as an IPOTESI (it is ABC in causal form); (d) *belief* — qualitative (low/medium/high), moved `UP`, `DOWN` or `UNCHANGED` by evidence weighed for independence (restatement · shared source · correlated · independent — SKILL step 6a), never by the count of sources alone and never as a number. Convergence of hypotheses raises priority, not epistemic level. A lead does not rise to `maturing` with low belief and zero corroboration.
