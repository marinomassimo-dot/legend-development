...
- Success criterion: every record leaves the sweep with a stated class and the basis for it. A batch may legitimately yield no insight, hypothesis, safety clue, bridge or priority change; then it records why it is low-yield. Super-inference is a method to apply where it fits, never a way to fill an empty result.
