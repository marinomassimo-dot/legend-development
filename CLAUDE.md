...
8. **A local abstract corpus is a map, never the territory.** [`pubmed_corpus_harvest.py`](framework/scripts/pubmed_corpus_harvest.py) harvests every record a documented PubMed query returns — for WWOX, 706 with 693 abstracts — into a gitignored `files/corpus/` artefact that carries its own `evidential_status: NOT_EVIDENCE`. **Permitted:** triage and ranking, census (what exists against what has been read), and export pre-flight — checking whether an abstract carries a proposition *before* the full text is opened, so the anchor is captured while the document is open. **Forbidden:** treating it as a reading. **Un abstract non è una lettura** and the corpus does not reduce reading debt by one paper. A receipt naming it is refused by `fulltext_receipts.validate_receipt()` **before the append**, and a locator anchored to it by `deepdive_manifest.validate()` — in the writers, not only in [`test_abstract_corpus_is_not_evidence.py`](scripts/test_abstract_corpus_is_not_evidence.py), because a test that is not run blocks nothing. Each locator also declares its **`surface`** (`body`/`figure`/`table`/`supplement`/`abstract`): naming the right artefact and quoting the abstract are different facts, and only the first used to be checked. The hazard is not that anyone decides to cut corners — it is that hundreds of abstracts sitting locally and greppable make answering from them *feel* like working. Rule 4 already forbids grep as a method of analysis; this is the same rule pointed at the corpus. The harvest is also not "every paper on the gene": PubMed expands a free-text query by Automatic Term Mapping, and the manifest records what it actually ran.
