...
| `analysis/data/dismech_authored_assertions.json` | Repository-authored source table for the first Phase-2 derivation: candidate selection, atomisation judgement, propositions, contexts and locator decisions previously embedded as Python literals | PUBLIC-DERIVED + THIRD-PARTY EXTRACTS | Externalised 2026-08-04 without changing the generated sidecar bytes; SHA-256 `833442bafba2149571502c9f4e328a1beb7c6fbb8017cd2ef530981ac6e7efd2`; sealed by the Git-anchored baseline | Repository-authored analytical judgements plus the same minimal PAPER 055/PAPER 056 verbatim snippets already declared for the first sidecar. Copyright remains with the article authors and publishers; no full text is redistributed. Non-canonical methodological input, not a scientific registry. |
| `analysis/data/dismech_phase2_baseline.json` | Repository-authored Git-anchored integrity seal of the Phase-2 derivation and blind-packet definition: SHA-256 of every fixed derivation/protocol input and sidecar; append-only prefix identity for the receipt ledger; occurrence, accounting and dedup invariants | PUBLIC-DERIVED | Frozen blobs are retrieved from ancestor commit `6d428905252fd0ab45286dae03402e800809f89c` with `git cat-file`; the baseline itself must match `HEAD`; receipt events 1–38 are sealed while later append-only events are permitted | Repository licence. Hashes and identifiers only; contains no third-party text. Git provides byte recovery and exposes any new trust root as reviewed commit history; a source archive without `.git` cannot perform this check. |
...
| `analysis/data/dismech_second_derivation_reconciled_016_024_035.jsonl` | Deterministic provenance reconciliation of the independently authored output after two targeted locator receipts were persisted | PUBLIC-DERIVED + THIRD-PARTY EXTRACTS | Authored bytes remain separately attested; reconciliation SHA-256 `33f2db6c7e933b6c147cde8e8461f8ddd8066b46eed2d287a86f1094acb6de23`; only receipt ID, terminal state and unreached-test fields may differ | Same minimal short extracts and attribution conditions as the authored second derivation. Methodological comparison only; not a curation decision. |
| `analysis/data/dismech_second_derivation_reconciliation_attestation_016_024_035.json` | Hash and mutable-field attestation for the deterministic reconciliation above | PUBLIC-DERIVED | Names locator receipts `FTR-20260804-35716775-05` and `FTR-20260804-22193544-05` and declares full-ancestry validation | Repository licence. Hashes, field names and receipt identifiers only; no article text. |
| `analysis/data/pathograph_export.jsonl` | Machine-readable form of the pathograph inventory: the claim nodes, the claim→claim links the registries declare, the relational propositions already written in claim titles, in the working-model claim mirror and in the deep-dive work manifests, and the classified absences. Nothing in it is authored — every record is a transformation of a declared input, and no relationship, endpoint or relation type is added | PUBLIC-DERIVED + THIRD-PARTY EXTRACTS | Generated by `framework/scripts/pathograph.py` from `claim_registry_current.md`, `paper_registry_current.md`, `working_model_current.md` and every `research/deepdive_manifests/PMID*.json`; the derivation manifest on line 1 carries the input digest, and `--verify` re-derives the file and fails on drift | Repository licence for the assembly, the classifications and the lexicon. **Locator-backed candidate records carry the `evidence.snippet` verbatim extracts already declared for the work manifests they come from**; copyright in those extracts remains with their authors and publishers, they are minimal and attributed by PMID with a section anchor, and no full text is redistributed. Candidate records are review material awaiting Scientist evidence review — not edges, not claims and not curation decisions. |
| `analysis/data/dismech_axis3_review_raw_reply_round2.md` | Verbatim reply of the second blind axis-3 reviewer: 40 verdicts with one-line justifications, per-anchor split agreement and the tally | PUBLIC-DERIVED | Produced 2026-08-04 by a fresh session with no repository access, from the recorded round-2 prompt; sides shuffled with a new seed | Repository licence. Contains the repository-authored propositions under review and judgements about them; no article text and no source extracts. Preserved unedited: the inter-rater comparison is only meaningful against the original wording. |
| `analysis/data/dismech_axis3_blinding_key_round2.json` | Round-2 side mapping for the axis-3 review, with a new seed so it cannot be aligned with round 1, plus the record of which three anchors were repaired between rounds and which eight were left untouched | PUBLIC-DERIVED | Generated 2026-08-04, seed `20260805`, shuffled independently per anchor | Repository licence. Labels, seed and anchor names only; no proposition text and no article text. Records that agreement at the repaired anchors is close to circular, so the file cannot be read as if all eleven carried equal weight. |
| `analysis/data/dismech_axis3_review_raw_reply.md` | Verbatim reply of the blind axis-3 reviewer: 37 verdicts with one-line justifications, per-anchor split agreement and the tally | PUBLIC-DERIVED | Produced 2026-08-04 by a fresh session with no repository access, from the recorded prompt; sides shuffled per anchor | Repository licence. Contains the repository-authored propositions under review and judgements about them; no article text and no source extracts. Preserved unedited because a tally can be recomputed and a justification cannot. |
| `analysis/data/dismech_axis3_blinding_key.json` | Mapping from the blinded sides `α`/`β` back to the reference and independent derivations, per anchor, plus the normalisations applied and the residual tells that survive them | PUBLIC-DERIVED | Generated 2026-08-04, seed `20260804`, shuffled independently per anchor | Repository licence. Labels, seed and method only; contains no proposition text and no article text. It exists to make the blinding auditable after review, and reading it beforehand defeats the blinding. |
| `analysis/data/dismech_second_derivation_realigned_016_024_035.jsonl` | Mechanical re-indexing of the independent derivation: `registry_anchor` `sent[n]` → `sent[n-1]`, because the anchor format did not declare its sentence-ordinal base and the two runs indexed from different origins. Propositions, contexts, locators, epistemic types and relations are copied verbatim — no re-authoring | PUBLIC-DERIVED + THIRD-PARTY EXTRACTS | Derived 2026-08-04 by `analysis/scripts/realign_second_derivation.py` from the authored output, which is unchanged and still hashes to `ec117b0a3b7194ff283dc9e16dc081817cafbbe2a7949409403f8586a446e07b`; the shift was confirmed on 31 of 31 records before writing | Same minimal short extracts and attribution conditions as the authored second derivation. Each record retains its pre-realignment anchor and occurrence identifier. A re-indexing, not a curation decision. |
| `analysis/data/dismech_second_derivation_realigned_016_024_035_manifest.json` | Transformation record for the re-indexing above: source and output hashes, the fields left verbatim, and the count of anchors confirmed | PUBLIC-DERIVED | Written alongside the realigned file, 2026-08-04 | Repository licence. Hashes and field names only; no article text. |
| `analysis/data/dismech_independent_comparison_rev12.json` | Machine-generated four-axis comparison of the first and independent reconciled derivations | PUBLIC-DERIVED | Generated 2026-08-04 by the Git-anchored Rev. 12 comparator; SHA-256 `6f83e12f4ec4d6454bea0250b75b411c1f7006264d639030e13e6e89908ef5ed` | Repository licence. Includes repository-authored propositions and short source extracts inherited from the two declared sidecars; no full text. Diagnostic output, not semantic equivalence or a scientific verdict. |
