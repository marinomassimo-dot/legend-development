...
| `analysis/data/dismech_phase2_baseline.json` | Repository-authored integrity seal of the Phase-2 derivation and blind-packet definition: SHA-256 of every fixed derivation input, sidecar, repository allowlist, generated receipt projection and blind contract; append-only prefix identity for the receipt ledger; occurrence, accounting and dedup invariants | PUBLIC-DERIVED | Sealed 2026-08-04 at git HEAD `8ab8e4b`; claim and paper registries are byte-identical to that tree; receipt events 1–36 are sealed while later append-only events are permitted | Repository licence. Hashes and identifiers only; contains no third-party text. Integrity seal, not a substitute for version-controlled or content-addressed byte recovery. |
