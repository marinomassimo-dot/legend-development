...
| `analysis/data/dismech_phase2_baseline.json` | Repository-authored Git-anchored integrity seal of the Phase-2 derivation and blind-packet definition: SHA-256 of every fixed derivation/protocol input and sidecar; append-only prefix identity for the receipt ledger; occurrence, accounting and dedup invariants | PUBLIC-DERIVED | Frozen blobs are retrieved from ancestor commit `91a92e5f590e95471806ffd3555d1fe754b1c4c2` with `git cat-file`; the baseline itself must match `HEAD`; receipt events 1–36 are sealed while later append-only events are permitted | Repository licence. Hashes and identifiers only; contains no third-party text. Git provides byte recovery and exposes any new trust root as reviewed commit history. |
